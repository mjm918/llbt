...
void ArrayString::init_from_mem(MemRef mem) noexcept
{
    ...
    if (!long_strings) {
        ...
        if (is_small) {
            ...
            // llbt scope cut: enum-strings is a table-layer column format; no
            // llbt-created file contains it.
            static_cast<void>(mem);
            LLBT_TERMINATE("enum-strings column encountered; table layer not present");
        }
    }
    else {
