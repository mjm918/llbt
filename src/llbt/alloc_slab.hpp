...
/// (or memory buffer) does not appear to contain a valid Barq
/// database.
struct InvalidDatabase;


/// The allocator that is used to manage the memory of a Barq
/// group, i.e., a Barq database.
///
...
public:
    ...
    /// If the file being opened is not a valid Barq file (possibly due to a
    ...
    /// Barq file (file system inode) is modified asynchronously.
    ///
    ...
    /// application to ensure that the Barq file is not modified concurrently
...
private:
    ...
    /// Throws InvalidDatabase if the file is not a Barq file, if the file is
    ...
    InvalidDatabase(const std::string& msg, const std::string& path)
        : FileAccessError(ErrorCodes::InvalidDatabase,
                          path.empty() ? "Failed to memory buffer:" + msg
                                       : util::format("Failed to open Barq file at path '%1': %2", path, msg),
                          path)
    {
    }
};
...
{
    LLBT_ASSERT_3(m_attach_mode, ==, attach_UsersBuffer);
    LLBT_ASSERT(m_data);
...
void SlabAlloc::for_all_free_entries(Func f) const
{
    ...
    for (const auto& e : m_slabs) {
        ...
        LLBT_ASSERT(bb->block_before_size == 0);
        while (1) {
