...
#ifndef LLBT_REPLICATION_HPP
#define LLBT_REPLICATION_HPP
...
// llbt scope cut: table-layer instrumentation removed.
    ...
    ///
    /// 1. The parent Transaction acquires exclusive write access to the local Barq.
    /// 2. initiate_transact() is called and succeeds.
    /// 3. Mutations in the Barq occur, each of which is reported to
    ...
    ///    Barq file itself, this must be called while the write transaction is
    ...
    /// 5. The parent Transaction object performs the commit operation on the local Barq.
    /// 6. finalize_commit() is called by the Transaction object. With
    ///    out-of-Barq replication logs this was used to mark the logs written in
    ///    step 4 as being valid. With modern in-Barq storage it is merely used
    ...
    /// CAUTION: These values are stored in Barq files, so value reassignment
    /// is not allowed.
    enum HistoryType {
        ...
        /// Out-of-Barq history supporting continuous transactions.
        ///
        /// NOTE: This history type is no longer in use. The value needs to stay
        /// reserved in case someone tries to open an old Barq file.
        hist_OutOfBarq = 1,

        /// In-Barq history supporting continuous transactions
        /// (make_in_barq_history()).
        hist_InBarq = 2,

        /// In-Barq history supporting continuous transactions and client-side
        /// synchronization protocol (llbt::sync::ClientHistory).
        hist_SyncClient = 3,

        /// In-Barq history supporting continuous transactions and server-side
        /// synchronization protocol (llbt::_impl::ServerHistory).
        hist_SyncServer = 4
    };
    ...
    /// history type, and that the Barq file contains a compatible type of
    ...
    /// is successfully committed, the Barq contains at least a top array, and
    ...
    ///
    /// For the sake of backwards compatibility with older Barq files that does
    /// not store any history type, the following rule shall apply:
    ///
    ///   - If the top array of a Barq file (Group::m_top) does not contain a
    ...
    /// preceding session that modified the Barq by sucessfully committing a
    /// new snapshot.
    ///
    /// It shall be allowed to switch to a \ref hist_InBarq history if the
    /// stored history type is \ref hist_None. This can be done simply by adding
    /// a new history to the Barq file. This is possible because histories of
    /// this type a transient in nature, and need not survive from one session
    /// to the next.
    ///
    /// On the other hand, as soon as a history of type \ref hist_InBarq is
    /// added to a Barq file, that history type is binding for all subsequent
    ...
    /// nature of it, however, because the \ref hist_InBarq history remains in
    /// the Barq file, there are practical complications, and for that reason,
    ...
    /// Barq during the first successfully committed transaction in that
