...
#ifdef LLBT_SLAB_ALLOC_DEBUG
#include <cstdlib>
#endif

#include <llbt/util/encrypted_file_mapping.hpp>
#include <llbt/util/errno.hpp>
#include <llbt/util/scope_exit.hpp>
...
public:
    ...
    {
        return "Free space tracking was lost due to out-of-memory. The Barq file must be closed and reopened before "
...
void SlabAlloc::init_streaming_header(Header* streaming_header, int file_format_version)
{
    ...
    LLBT_ASSERT(!util::int_cast_has_overflow<storage_type>(file_format_version));
    *streaming_header = {
    ...
    , size(s)
{
    ...
    addr = new char[size];
    LLBT_ASSERT((reinterpret_cast<size_t>(addr) & 0x7ULL) == 0);
#if LLBT_ENABLE_ALLOC_SET_ZERO
    ...
    if (is_attached()) {
        ...
        if (m_attach_mode != attach_SharedFile) {
            ...
            if (m_free_space_state != free_space_Invalid) {
                if (LLBT_COVER_NEVER(!is_all_free())) {
                    print();
#ifndef LLBT_SLAB_ALLOC_DEBUG
                    std::cerr << "To get the stack-traces of the corresponding allocations,"
                                 "first compile with LLBT_SLAB_ALLOC_DEBUG defined,"
                                 "then run under Valgrind with --leak-check=full\n";
                    LLBT_TERMINATE("SlabAlloc detected a leak");
#endif
                }
            }
        }
...
MemRef SlabAlloc::do_alloc(size_t size)
{
    ...
    LLBT_ASSERT_EX(0 < size, size, get_file_path_for_assertions());
    LLBT_ASSERT_EX((size & 0x7) == 0, size,
                    get_file_path_for_assertions()); // only allow sizes that are multiples of 8
    LLBT_ASSERT_EX(is_attached(), get_file_path_for_assertions());
    ...
    LLBT_ASSERT_RELEASE_EX(size < (1 << section_shift), size, get_file_path_for_assertions());

    // If we failed to correctly record free space, new allocations cannot be
    // carried out until the free space record is reset.
    if (LLBT_COVER_NEVER(m_free_space_state == free_space_Invalid))
...
#ifdef LLBT_DEBUG
    if (LLBT_COVER_NEVER(m_debug_out))
        std::cerr << "Alloc ref: " << ref << " size: " << size << "\n";
#endif

    char* addr = reinterpret_cast<char*>(entry);
    LLBT_ASSERT_EX(addr == translate(ref), addr, ref, get_file_path_for_assertions());
...
#endif
    LLBT_ASSERT_EX(ref >= m_baseline, ref, m_baseline, get_file_path_for_assertions());
...
void SlabAlloc::remove_freelist_entry(FreeBlock* entry)
{
    ...
    LLBT_ASSERT_EX(it != m_block_map.end(), get_file_path_for_assertions());
...
{
    auto bb = bb_before(entry);
    LLBT_ASSERT_EX(bb->block_after_size < 0, bb->block_after_size, get_file_path_for_assertions());
    auto alloc_size = -bb->block_after_size;
    int max_waste = sizeof(FreeBlock) + sizeof(BetweenBlocks);
    LLBT_ASSERT_EX(alloc_size >= size && alloc_size <= size + max_waste, alloc_size, size,
                    get_file_path_for_assertions());
    bb->block_after_size = alloc_size;
    bb = bb_after(entry);
    LLBT_ASSERT_EX(bb->block_before_size < 0, bb->block_before_size, get_file_path_for_assertions());
    LLBT_ASSERT(-bb->block_before_size == alloc_size);
...
void SlabAlloc::mark_allocated(FreeBlock* entry)
{
    ...
    LLBT_ASSERT_EX(bb2->block_before_size > 0, bb2->block_before_size, get_file_path_for_assertions());
...
SlabAlloc::FreeBlock* SlabAlloc::allocate_block(int size)
{
    ...
    LLBT_ASSERT_EX(size_from_block(block) >= size, size_from_block(block), size, get_file_path_for_assertions());
    return block;
}
...
SlabAlloc::FreeBlock* SlabAlloc::grow_slab(int size)
{
    ...
    if (m_slabs.empty()) {
        ...
        LLBT_ASSERT_DEBUG_EX(curr_ref_end >= m_baseline, curr_ref_end, m_baseline, get_file_path_for_assertions());
        ref = curr_ref_end;
    }
    ref = align_size_to_section_boundary(ref);
    size_t ref_end = ref;
    if (LLBT_UNLIKELY(int_add_with_overflow_detect(ref_end, new_size))) {
        throw MaximumFileSizeExceeded("AllocSlab slab ref_end size overflow: " + util::to_string(ref) + " + " +
                                      util::to_string(new_size));
    }

    LLBT_ASSERT(matches_section_boundary(ref));
...
{
    LLBT_ASSERT_EX(translate(ref) == addr, translate(ref), addr, get_file_path_for_assertions());
    CriticalSection cs(changes);
    ...
    LLBT_ASSERT_EX(read_only || m_free_space_state == free_space_Dirty, read_only, m_free_space_state,
    ...
    if (read_only) {
        ...
        try {
            LLBT_ASSERT_RELEASE_EX(ref != 0, ref, get_file_path_for_assertions());
            LLBT_ASSERT_RELEASE_EX(!(ref & 7), ref, get_file_path_for_assertions());
            auto next = m_free_read_only.lower_bound(ref);
            if (next != m_free_read_only.end()) {
                LLBT_ASSERT_RELEASE_EX(ref + size <= next->first, ref, size, next->first, next->second,
                                        get_file_path_for_assertions());
            ...
            if (!m_free_read_only.empty() && next != m_free_read_only.begin()) {
                ...
                LLBT_ASSERT_RELEASE_EX(prev->first + prev->second <= ref, ref, size, prev->first, prev->second,
                                        get_file_path_for_assertions());
        ...
        LLBT_ASSERT_RELEASE_EX(size < 2UL * 1024 * 1024 * 1024, size, get_file_path_for_assertions());
...
{
    LLBT_ASSERT_DEBUG(translate(ref) == addr);
    LLBT_ASSERT_EX(0 < new_size, new_size, get_file_path_for_assertions());
    LLBT_ASSERT_EX((new_size & 0x7) == 0, new_size,
                    get_file_path_for_assertions()); // only allow sizes that are multiples of 8
    ...
    // fill with zero if LLBT_ENABLE_ALLOC_SET_ZERO is non-zero.
    ...
    llbt::safe_copy_n(addr, old_size, new_addr);
...
{
    LLBT_ASSERT(false); // never come here
    return nullptr;
}
...
bool SlabAlloc::align_filesize_for_mmap(ref_type top_ref, Config& cfg)
{
    ...
    LLBT_ASSERT(!is_file_on_streaming_form());
...
ref_type SlabAlloc::attach_file(const std::string& path, Config& cfg, util::WriteObserver* write_observer)
{
    ...
    LLBT_ASSERT_EX(!(cfg.is_shared && cfg.read_only), cfg.is_shared, cfg.read_only, get_file_path_for_assertions());
    // session_initiator can be set *only* if we're shared.
    LLBT_ASSERT_EX(cfg.is_shared || !cfg.session_initiator, cfg.is_shared, cfg.session_initiator,
                    get_file_path_for_assertions());
    // clear_file can be set *only* if we're the first session.
    LLBT_ASSERT_EX(cfg.session_initiator || !cfg.clear_file, cfg.session_initiator, cfg.clear_file,
                    get_file_path_for_assertions());
    ...
    try {
        ...
        auto msg = util::format_errno("Failed to open Barq file at path '%2': %1", ex.get_errno(), path);
    ...
    // size_t.
    if (LLBT_UNLIKELY(int_cast_with_overflow_detect(m_file.get_size(), size)))
        throw InvalidDatabase("Barq file too large", path);
    ...
    if (size == 0) {
        if (LLBT_UNLIKELY(cfg.read_only))
            throw InvalidDatabase("Read-only access to empty Barq file", path);
...
void SlabAlloc::convert_from_streaming_form(ref_type top_ref)
{
    ...
    // session initialization, even if it means writing the database during open.
    {
        ...
        llbt::util::encryption_write_barrier(writable_map, 0);
        writable_map.sync();
        util::encryption_read_barrier(writable_map, 0);
...
ref_type SlabAlloc::attach_buffer(const char* data, size_t size)
{
    ...
    LLBT_ASSERT_EX(!is_attached(), get_file_path_for_assertions());
    LLBT_ASSERT_EX(size <= (1UL << section_shift), get_file_path_for_assertions());
...
char* SlabAlloc::translate_memory_pos(ref_type ref) const noexcept
{
    ...
    LLBT_ASSERT(idx < m_virtual_file_buffer.size());
                                             ...
                                             bool session_initiator, util::WriteObserver* write_observer)
{
    try {
        ...
        if (session_initiator && is_file_on_streaming_form(*header)) {
            ...
            LLBT_ASSERT_EX(header->m_flags == 0, header->m_flags, path);
            LLBT_ASSERT_EX(header->m_mnemonic[0] == uint8_t('T'), header->m_mnemonic[0], path);
            LLBT_ASSERT_EX(header->m_mnemonic[1] == uint8_t('-'), header->m_mnemonic[1], path);
            LLBT_ASSERT_EX(header->m_mnemonic[2] == uint8_t('D'), header->m_mnemonic[2], path);
            LLBT_ASSERT_EX(header->m_mnemonic[3] == uint8_t('B'), header->m_mnemonic[3], path);
            LLBT_ASSERT_EX(header->m_top_ref[0] == 0xFFFFFFFFFFFFFFFFULL, header->m_top_ref[0], path);
            LLBT_ASSERT_EX(header->m_top_ref[1] == 0, header->m_top_ref[1], path);
            LLBT_ASSERT_EX(footer->m_magic_cookie == footer_magic_cookie, footer->m_magic_cookie, path);
        }
        return top_ref;
    }
    catch (const InvalidDatabase&) {
        ...
        throw InvalidDatabase(util::format("Barq file decryption failed (%1)", e.what()), path);
    }
                                    ...
                                    const std::string& path, bool is_encrypted)
{
    ...
    if (LLBT_UNLIKELY(size < sizeof(Header)))
        throw InvalidDatabase(util::format("file is non-empty but too small (%1 bytes) to be a valid Barq.", size),
                              path);
    if (LLBT_UNLIKELY(size % 8 != 0))
        throw InvalidDatabase(util::format("file has an invalid size (%1).", size), path);

    // First four bytes of info block is file format id
    if (LLBT_UNLIKELY(!(char(header->m_mnemonic[0]) == 'T' && char(header->m_mnemonic[1]) == '-' &&
        ...
        if (is_encrypted) {
            ...
            throw_header_exception("header has invalid mnemonic. The file does not appear to be Barq file.", *header,
                                   path);
        }
        else {
            throw_header_exception("header has invalid mnemonic. The file is either not a Barq file, is an "
                                   "encrypted Barq file but no encryption key was supplied, or is corrupted.",
                                   *header, path);
        }
    }
    ...
    if (slot_selector == 0 && top_ref == 0xFFFFFFFFFFFFFFFFULL) {
        if (LLBT_UNLIKELY(size < sizeof(Header) + sizeof(StreamingFooter))) {
            throw InvalidDatabase(
                util::format("file is in streaming format but too small (%1 bytes) to be a valid Barq.", size),
                path);
        }
        LLBT_ASSERT(footer);
        top_ref = footer->m_top_ref;
        if (LLBT_UNLIKELY(footer->m_magic_cookie != footer_magic_cookie)) {
    ...
    if (LLBT_UNLIKELY(top_ref % 8 != 0)) {
        throw_header_exception("top ref is not aligned", *header, path);
    }
    if (LLBT_UNLIKELY(top_ref >= size)) {
...
void SlabAlloc::reset_free_space_tracking()
{
    ...
    while (m_slabs.size() > 1 || (m_slabs.size() == 1 && m_slabs[0].size > minimal_alloc)) {
        ...
        LLBT_ASSERT(last_translation.mapping_addr == last_slab.addr);
...
void SlabAlloc::update_reader_view(size_t file_size)
{
    ...
    if (!is_in_memory()) {
        LLBT_ASSERT_EX(file_size % 8 == 0, file_size, get_file_path_for_assertions()); // 8-byte alignment required
        LLBT_ASSERT_EX(m_attach_mode == attach_SharedFile || m_attach_mode == attach_UnsharedFile, m_attach_mode,
                        get_file_path_for_assertions());
        LLBT_ASSERT_DEBUG(is_free_space_clean());
        ...
        LLBT_ASSERT(m_mappings.size() == old_num_mappings);

        {
            ...
            if (old_baseline < old_slab_base) {
                ...
                LLBT_ASSERT(old_num_mappings > 0);
        ...
        if (replace_last_mapping) {
            ...
            LLBT_ASSERT(!cur_entry.xover_mapping.is_attached());
    ...
    // until the barq version for which they were established has been closed/detached.
    //
...
void SlabAlloc::rebuild_translations(bool requires_new_translation, size_t old_num_sections)
{
    ...
    for (size_t i = old_num_sections; i < num_mappings; ++i) {
        ...
        LLBT_ASSERT(new_translation_table[i].mapping_addr);
...
void SlabAlloc::get_or_add_xover_mapping(RefTranslation& txl, size_t index, size_t offset, size_t size)
{
    ...
    if (txl.xover_mapping_addr.load(std::memory_order_relaxed)) {
        ...
        LLBT_ASSERT(offset == txl.lowest_possible_xover_offset.load(std::memory_order_relaxed));
        return;
    }
    MapEntry* map_entry = &m_mappings[index];
    LLBT_ASSERT(map_entry->primary_mapping.get_addr() == txl.mapping_addr);
...
void SlabAlloc::verify_old_translations(uint64_t youngest_live_version)
{
    ...
    for (auto& m : m_old_mappings) {
        LLBT_ASSERT(m.mapping.is_attached());
        mappings.insert(m.mapping.get_addr());
    }
    for (auto& m : m_mappings) {
        LLBT_ASSERT(m.primary_mapping.is_attached());
    ...
    for (auto& t : m_old_translations) {
        LLBT_ASSERT_EX(youngest_live_version == 0 || t.replaced_at_version < youngest_live_version,
                        youngest_live_version, t.replaced_at_version);
        if (nonempty_attachment()) {
            for (size_t i = 0; i < t.translation_count; ++i)
                LLBT_ASSERT(mappings.count(t.translations[i].mapping_addr));
        }
    }
#else
...
{
    if (m_attach_mode == attach_SharedFile) {
        LLBT_ASSERT_EX(new_file_size == round_up_to_page_size(new_file_size), get_file_path_for_assertions());
        ...
        LLBT_ASSERT_EX(new_file_size <= static_cast<size_t>(m_file.get_size()), new_file_size, m_file.get_size());
...
void SlabAlloc::verify() const
{
    ...
    if (!m_slabs.empty()) {
        ...
        for_all_free_entries([&](size_t ref, size_t size) {
            LLBT_ASSERT(ref >= first_possible_ref);
            LLBT_ASSERT(ref + size <= first_impossible_ref);
...
bool SlabAlloc::is_all_free() const
{
    ...
    for (const auto& e : m_slabs) {
        ...
        LLBT_ASSERT(first->block_before_size == 0);
        auto last = reinterpret_cast<BetweenBlocks*>(e.addr + e.size) - 1;
        LLBT_ASSERT(last->block_after_size == 0);
