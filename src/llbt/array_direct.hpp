...
#ifndef LLBT_ARRAY_DIRECT_HPP
#define LLBT_ARRAY_DIRECT_HPP
...
#define LLBT_TEMPEX(fun, wid, arg) \
    ...
    else {LLBT_ASSERT_DEBUG(false); fun<0> arg;}

#define LLBT_TEMPEX2(fun, targ, wid, arg) \
    ...
    else {LLBT_ASSERT_DEBUG(false); fun<targ, 0> arg;}

#define LLBT_TEMPEX3(fun, targ1, wid, targ3, arg) \
    ...
    else {LLBT_ASSERT_DEBUG(false); fun<targ1, 0, targ3> arg;}

#define LLBT_TEMPEX4(fun, targ1, targ3, targ4, wid, arg) \
    ...
    else {LLBT_ASSERT_DEBUG(false); fun<targ1, targ3, targ4, 0> arg;}
// clang-format on
...
{
    if (width == 0) {
        LLBT_ASSERT_DEBUG(value == 0);
        return;
    }
    else if (width == 1) {
        LLBT_ASSERT_DEBUG(0 <= value && value <= 0x01);
        ...
        *p = uchar((*p & ~(0x01 << bit_ndx)) | (int(value) & 0x01) << bit_ndx);
        ...
        LLBT_ASSERT_DEBUG(0 <= value && value <= 0x03);
        ...
        *p = uchar((*p & ~(0x03 << bit_ndx)) | (int(value) & 0x03) << bit_ndx);
        ...
        LLBT_ASSERT_DEBUG(0 <= value && value <= 0x0F);
        ...
        *p = uchar((*p & ~(0x0F << bit_ndx)) | (int(value) & 0x0F) << bit_ndx);
        ...
        LLBT_ASSERT_DEBUG(std::numeric_limits<int8_t>::min() <= value &&
                           value <= std::numeric_limits<int8_t>::max());
        *(reinterpret_cast<int8_t*>(data) + ndx) = int8_t(value);
    }
    else if (width == 16) {
        LLBT_ASSERT_DEBUG(std::numeric_limits<int16_t>::min() <= value &&
                           value <= std::numeric_limits<int16_t>::max());
        *(reinterpret_cast<int16_t*>(data) + ndx) = int16_t(value);
    }
    else if (width == 32) {
        LLBT_ASSERT_DEBUG(std::numeric_limits<int32_t>::min() <= value &&
                           value <= std::numeric_limits<int32_t>::max());
        *(reinterpret_cast<int32_t*>(data) + ndx) = int32_t(value);
    }
    else if (width == 64) {
        LLBT_ASSERT_DEBUG(std::numeric_limits<int64_t>::min() <= value &&
...
{
    LLBT_TEMPEX(set_direct, width, (data, ndx, value));
}
...
{
    LLBT_TEMPEX(return get_direct, width, (data, ndx));
}
...
{
    LLBT_TEMPEX(return get_two, width, (data, ndx));
}
