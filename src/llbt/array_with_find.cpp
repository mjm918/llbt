...
{
    LLBT_ASSERT_3(begin, <=, m_array.size());
    LLBT_ASSERT(end == npos || (begin <= end && end <= m_array.size()));

    if (end == npos)
        end = m_array.m_size;

    QueryStateFindAll state(*result);
    LLBT_TEMPEX2(find_optimized, Equal, m_array.m_width, (value, begin, end, col_offset, &state));
