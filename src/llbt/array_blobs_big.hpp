...
#ifndef LLBT_ARRAY_BIG_BLOBS_HPP
#define LLBT_ARRAY_BIG_BLOBS_HPP
...
{
    LLBT_ASSERT_DEBUG(!(!m_nullable && value.is_null()));
                                            ...
                                            bool nullable = true) noexcept
{
    ...
    LLBT_ASSERT_DEBUG(!(!nullable && bin.is_null()));
    if (bin.is_null())
        return llbt::null();
    else
        return StringData(bin.data(), bin.size() - 1); // Do not include terminating zero
...
#endif // LLBT_ARRAY_BIG_BLOBS_HPP
