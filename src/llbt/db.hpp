...
#include <llbt/util/functional.hpp>
#include <llbt/util/interprocess_condvar.hpp>
#include <llbt/util/interprocess_mutex.hpp>
#include <llbt/util/encrypted_file_mapping.hpp>
#include <llbt/version_id.hpp>
...
struct IncompatibleLockFile : FileAccessError {
    IncompatibleLockFile(const std::string& path, const std::string& msg)
        : FileAccessError(
              ...
              util::format(
                  "Barq file '%1' is currently open in another process which cannot share access with this process. "
                  "This could either be due to the existing process being a different architecture or due to the "
                  "existing process using an incompatible version of Barq. "
                  "If the other process is Barq Studio, you may need to update it (or update Barq if your Studio "
...
/// (Replication::HistoryType) in the opened Barq file is incompatible with the
/// mode in which the Barq file is opened. For example, if there is a mismatch
...
struct IncompatibleHistories : FileAccessError {
    IncompatibleHistories(const std::string& msg, const std::string& path)
        : FileAccessError(ErrorCodes::IncompatibleHistories,
                          msg + " Synchronized Barqs cannot be opened in non-sync mode, and vice versa.", path)
    {
    }
};
...
/// It will also be thrown if a barq which requires upgrade is opened in read-only
/// mode (Group::open).
...
///
/// Barq has 3 types of Transactions:
...
public:
    ...
    // file is used to set the `db_path` used to register and associate a users's SyncSession with the Barq path (see
    ...
    using VersionID = llbt::VersionID;
    ...
    // Try to grab an exclusive lock of the given barq path's lock file. If the lock
    ...
    // files through a DB.
    // It is safe to delete/replace barq files inside the callback.
    // WARNING: It is not safe to delete the lock file in the callback.
    using CallbackWithLock = util::FunctionRef<void(const std::string& llbt_path)>;
    static bool call_with_lock(const std::string& llbt_path, CallbackWithLock&& callback);
    ...
    /// Get the path for the given type of file for a base Barq file path.
    /// \param llbt_path The path for the main Barq file.
    ...
    static std::string get_core_file(const std::string& llbt_path, CoreFileType type);

    /// Delete a Barq file and all associated control files.
    ///
    ...
    ///
    /// \param base_path The Barq file to delete, which auxiliary file paths will be derived from.
    /// \param[out] did_delete_barq If non-null, will be set to true if the Barq file was deleted (even if a
    ...
    static void delete_files(const std::string& base_path, bool* did_delete_barq = nullptr,
...
private:
    ...
    struct ReadLockInfo {
        ...
        {
            LLBT_ASSERT_RELEASE_EX((m_top_ref & 7) == 0 && m_top_ref < m_file_size, m_version, m_reader_idx,
    ...
    /// thrown.
    ///
    /// \param file Filesystem path to a Barq database file.
    ///
    ...
    /// history schema version is one which this version of Barq does not know
    /// how to migrate from.
