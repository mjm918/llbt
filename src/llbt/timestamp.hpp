...
#ifndef LLBT_TIMESTAMP_HPP
#define LLBT_TIMESTAMP_HPP
...
public:
    ...
    constexpr Timestamp(int64_t seconds, int32_t nanoseconds)
        ...
        , m_is_null(false)
    {
        LLBT_ASSERT_EX(-nanoseconds_per_second < nanoseconds && nanoseconds < nanoseconds_per_second, nanoseconds);
        const bool both_non_negative = seconds >= 0 && nanoseconds >= 0;
        const bool both_non_positive = seconds <= 0 && nanoseconds <= 0;
        LLBT_ASSERT_EX(both_non_negative || both_non_positive, both_non_negative, both_non_positive);
    }
    constexpr Timestamp() = default;
    constexpr Timestamp(llbt::null) {}
...
namespace std {
template <>
struct numeric_limits<llbt::Timestamp> {
    static constexpr bool is_integer = false;
    static constexpr llbt::Timestamp min()
    {
        return llbt::Timestamp(numeric_limits<int64_t>::min(), 0);
    }
    static constexpr llbt::Timestamp lowest()
    {
        return llbt::Timestamp(numeric_limits<int64_t>::lowest(), 0);
    }
    static constexpr llbt::Timestamp max()
    {
        return llbt::Timestamp(numeric_limits<int64_t>::max(), 0);
    }
};
} // namespace std

#endif // LLBT_TIMESTAMP_HPP
