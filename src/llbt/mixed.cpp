...
// llbt scope cut: Mixed(const Obj&) removed with the object layer.
...
int Mixed::compare(const Mixed& b) const noexcept
{
    ...
    switch (type) {
        ...
        default:
            ...
            LLBT_ASSERT_RELEASE(false && "Compare not supported for this column type");
            break;
    }

    // Comparing rank of types as a fallback makes it possible to sort of a list of Mixed
    LLBT_ASSERT(sorting_rank[m_type] != sorting_rank[b.m_type]);
...
size_t Mixed::hash() const
{
    ...
    switch (get_type()) {
        ...
        case type_Mixed:
        case type_Link:
            LLBT_ASSERT_RELEASE(false && "Hash not supported for this column type");
            break;
    }
...
StringData Mixed::get_index_data(std::array<char, 16>& buffer) const noexcept
{
    ...
    switch (type) {
        case type_Int: {
            ...
            llbt::safe_copy_n(c, sizeof(int64_t), buffer.data());
        ...
        case type_Float: {
            ...
            llbt::safe_copy_n(src, sizeof(float), buffer.data());
        ...
        case type_Double: {
            ...
            const char* src = reinterpret_cast<const char*>(&v2);
            llbt::safe_copy_n(src, sizeof(double), buffer.data());
        ...
        case type_Timestamp: {
            ...
            llbt::safe_copy_n(s_buf, sizeof(s), buffer.data());
            llbt::safe_copy_n(ns_buf, sizeof(ns), buffer.data() + sizeof(s));
        ...
        case type_Decimal: {
            ...
            const char* src = reinterpret_cast<const char*>(&v2);
            llbt::safe_copy_n(src, sizeof(v2), buffer.data());
        ...
        case type_TypedLink: {
            ...
            llbt::safe_copy_n(src, sizeof(k1), buffer.data());
            src = reinterpret_cast<const char*>(&k2);
            llbt::safe_copy_n(src, sizeof(k2), buffer.data() + sizeof(k1));
    ...
    LLBT_ASSERT_RELEASE(false && "Index not supported for this column type");
    return {};
}
