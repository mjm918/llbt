...
#ifndef LLBT_STRING_HPP
#define LLBT_STRING_HPP
...
/// string by Barq.
///
/// For compatibility with C-style strings, when a string is stored in a Barq
...
/// when a StringData object is extracted from Barq, the referenced region is
...
constexpr inline StringData::StringData(const char* external_data, size_t data_size) noexcept
    ...
    , m_size(data_size)
{
    LLBT_ASSERT_DEBUG(external_data || data_size == 0);
}
...
namespace std {
template <>
struct hash<::llbt::StringData> {
    inline size_t operator()(const ::llbt::StringData& str) const noexcept
    {
        return str.hash();
    }
};
