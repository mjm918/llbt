...
//
// `N_c` must always be a power of `LLBT_MAX_BPNODE_SIZE`.
//
...
// a barq file will never grow in size above what can be represented in
...
{
    LLBT_ASSERT_3(begin, <=, end);
    LLBT_ASSERT_3(end, <=, m_size);
    LLBT_ASSERT_3(dest_begin, <=, m_size);
    LLBT_ASSERT_3(end - begin, <=, m_size - dest_begin);
    LLBT_ASSERT(!(dest_begin >= begin && dest_begin < end)); // Required by std::copy
    ...
    llbt::safe_copy_n(begin_2, end_2 - begin_2, dest_begin_2);
}
...
{
    LLBT_ASSERT(value >= 0);
    LLBT_ASSERT(&m_alloc == &Allocator::get_default());
...
void Array::insert(size_t ndx, int_fast64_t value)
{
    ...
    if (do_expand) {
        ...
        LLBT_ASSERT_DEBUG(width > m_width);
...
{
    LLBT_TEMPEX(return sum, m_width, (start, end));
}
...
int64_t Array::sum(size_t start, size_t end) const
{
    ...
    LLBT_ASSERT_EX(end <= m_size && start <= end, start, end, m_size);
    ...
    if (w == 1 || w == 2 || w == 4) {
        ...
        for (size_t t = 0; t < chunks; t++) {
            if (w == 1) {
#if 0
#if defined(USE_SSE42) && defined(_MSC_VER) && defined(LLBT_PTR_64)
                s += __popcnt64(data[t]);
#elif !defined(_MSC_VER) && defined(USE_SSE42) && defined(LLBT_PTR_64)
...
{
    LLBT_ASSERT(width != 0 && (width & (width - 1)) == 0); // Is a power of two
...
MemRef Array::clone(MemRef mem, Allocator& alloc, Allocator& target_alloc)
{
    ...
    if (!get_hasrefs_from_header(header)) {
        ...
        llbt::safe_copy_n(src_begin, src_end - src_begin, dst_begin);
    ...
    LLBT_ASSERT_3(get_wtype_from_header(header), ==, wtype_Bits);
...
MemRef Array::create(Type type, bool context_flag, WidthType width_type, size_t size, int_fast64_t value,
                     Allocator& alloc)
{
    LLBT_ASSERT_7(value, ==, 0, ||, width_type, ==, wtype_Bits);
    LLBT_ASSERT_7(size, ==, 0, ||, width_type, !=, wtype_Ignore);
    ...
    if (value != 0) {
        ...
        LLBT_TEMPEX(fill_direct, width, (data, begin, end, value));
    }
...
void Array::update_width_cache_from_header() noexcept
{
    ...
    LLBT_TEMPEX(m_vtable = &VTableForWidth, width, ::vtable);
...
void Array::verify() const
{
    ...
    LLBT_ASSERT(m_width == 0 || m_width == 1 || m_width == 2 || m_width == 4 || m_width == 8 || m_width == 16 ||
    ...
    LLBT_ASSERT_3(ref_in_parent, ==, m_ref);
#endif
}

size_t Array::lower_bound_int(int64_t value) const noexcept
{
    LLBT_TEMPEX(return lower_bound, m_width, (m_data, m_size, value));
}

size_t Array::upper_bound_int(int64_t value) const noexcept
{
    LLBT_TEMPEX(return upper_bound, m_width, (m_data, m_size, value));
}
