...
#define LLBT_GROUP_SHARED_OPTIONS_HPP
...
struct DBOptions {
    ...
    /// The persistence level of the Barq file. See Durability.
    Durability durability = Durability::Full;

    /// The key to encrypt and decrypt the Barq file with, or nullptr to
    /// indicate that encryption should not be used.
    const char* encryption_key;

    /// If \a allow_file_format_upgrade is set to `true`, this function will
    /// automatically upgrade the file format used in the specified Barq file
    ...
    ///
    /// - the specified Barq file is already using the latest file format, and
    ///   can be used, or
    ///
    /// - the specified Barq file uses a deprecated file format, resulting a
    ...
    /// Barq file is upgraded. The two parameters in the function are the
    /// previous version and the version just upgraded to, respectively.
    /// If the callback function throws, the Barq file will safely abort the
    ...
    /// A path to a directory where Barq can write temporary files or pipes to.
...
#endif // LLBT_GROUP_SHARED_OPTIONS_HPP
