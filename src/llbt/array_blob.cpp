...
ref_type Array::blob_replace(size_t begin, size_t end, const char* data, size_t data_size, bool add_zero_term)
{
    ...
    LLBT_ASSERT(get_context_flag());
    size_t sz = blob_size();
    LLBT_ASSERT_3(begin, <=, end);
    LLBT_ASSERT_3(end, <=, sz);
    LLBT_ASSERT(data_size == 0 || data);

    LLBT_ASSERT((begin == 0 || begin == sz) && end == sz); // Only append or total replace supported
    if (begin == sz && end == sz) {
        // Append
...
ref_type ArrayBlob::replace(size_t begin, size_t end, const char* data, size_t data_size, bool add_zero_term)
{
    ...
    // Insert the data
    modify_begin = llbt::safe_copy_n(data, data_size, modify_begin);
    if (add_zero_term)
        *modify_begin = 0;
