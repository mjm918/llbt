...
#ifndef LLBT_KEYS_HPP
#define LLBT_KEYS_HPP

#include <llbt/util/to_string.hpp>
#include <llbt/column_type.hpp>
#include <ostream>
#include <vector>
...
template <>
struct hash<llbt::ObjKey> {
    size_t operator()(llbt::ObjKey key) const
    {
        return std::hash<uint64_t>{}(key.value);
    }
};
template <>
struct hash<llbt::ColKey> {
    size_t operator()(llbt::ColKey key) const
    {
        return std::hash<int64_t>{}(key.value);
    }
};
template <>
struct hash<llbt::TableKey> {
    size_t operator()(llbt::TableKey key) const
    {
