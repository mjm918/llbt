...
#ifndef LLBT_ARRAY_BASIC_TPL_HPP
#define LLBT_ARRAY_BASIC_TPL_HPP
...
{
    LLBT_ASSERT(type == NodeHeader::type_Normal);
    LLBT_ASSERT(!context_flag);
    size_t length = 0;
...
size_t BasicArrayNull<T>::find_first_null(size_t begin, size_t end) const
{
    ...
    LLBT_ASSERT(begin <= sz && end <= sz && begin <= end);
    while (begin != end) {
...
#endif // LLBT_ARRAY_BASIC_TPL_HPP
