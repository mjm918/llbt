...
//         fields.
//  8      Placing the commitlog history inside the Barq file.
...
struct VersionList {
    ...
    ReadCount* try_allocate_entry(uint64_t top, uint64_t size, uint64_t version)
    {
        ...
        auto& rc = data()[i];
        LLBT_ASSERT(rc.count_frozen == 0);
        LLBT_ASSERT(rc.count_live == 0);
        LLBT_ASSERT(rc.count_full == 0);
        rc.current_top = top;
        rc.filesize = size;
        rc.activate(version);
    ...
    void purge_versions(uint64_t& oldest_live_v, TopRefMap& top_refs, bool& any_new_unreachables)
    {
        ...
        for (auto* rc = data(); rc < data() + entries; ++rc) {
            ...
            if (rc->count_frozen == 0 && rc->count_live == 0 && rc->version < oldest_full_v) {
                ...
                LLBT_ASSERT(index_of(*rc) != index_of_newest);
                free_entry(rc);
            }
        }
        LLBT_ASSERT(oldest_v != std::numeric_limits<uint64_t>::max());
        LLBT_ASSERT(oldest_live_v != std::numeric_limits<uint64_t>::max());
    }
...
/// overlapping openings of a particular Barq file via DB objects. For
...
struct alignas(8) DB::SharedInfo {
    ...
    /// The target Barq file format version for the current session. This
    ...
    /// and only during the Barq file opening process. If it differs, it means
    ...
    , shared_controlmutex() // Throws
{
    ...
    LLBT_ASSERT(!util::int_cast_has_overflow<decltype(history_type)>(ht + 0));
    LLBT_ASSERT(!util::int_cast_has_overflow<decltype(history_schema_version)>(hsv));
    history_type = ht;
...
public:
    ...
    void release_read_lock(const ReadLockInfo& read_lock) REQUIRES(!m_local_readers_mutex, !m_info_mutex)
    {
        {
            util::CheckedLockGuard lock(m_local_readers_mutex);
            LLBT_ASSERT(read_lock.m_reader_idx < m_local_readers.size());
        ...
        LLBT_ASSERT(read_lock.m_reader_idx < m_local_max_entry);
        auto& r = m_info->readers.get(read_lock.m_reader_idx);
        LLBT_ASSERT(read_lock.m_version == r.version);
        ...
        REQUIRES(!m_local_readers_mutex, !m_info_mutex)
    {
...

        {
            ...
            LLBT_ASSERT(newest != VersionList::nil);
...

        {
            ...
            LLBT_ASSERT_EX(field_for_type(r2, type) == 0, type, r2.count_full, r2.count_live, r2.count_frozen);
    ...
    static uint32_t& field_for_type(VersionList::ReadCount& r, ReadLockInfo::Type type)
    {
        switch (type) {
            ...
            case ReadLockInfo::Full:
                return r.count_full;
            default:
                LLBT_UNREACHABLE(); // silence a warning
        }
    }
...
public:
        ...
        , m_file(file)
    {
        ...
        while (size < required_size) {
            ...
            LLBT_ASSERT(required_size >= size);
        }
    }
...
#pragma GCC diagnostic pop
        ...
        if (info->init_complete == 0)
            continue;
        LLBT_ASSERT(info->init_complete == 1);
        ...
        // - backup of the barq file in preparation of file format upgrade
        ...
        // - Waiting for and signalling database changes
        {
            ...
            if (backup.must_restore_from_backup(current_file_format_version)) {
                // we need to unmap before any file ops that'll change the barq
                // file:
            ...
            if (options.backup_at_file_format_change) {
                backup.backup_barq_if_needed(current_file_format_version, target_file_format_version);
            }
            using gf = _impl::GroupFriend;
            bool file_format_ok;
            // In shared mode (Barq file opened via a DB instance) this
            // version of the core library is able to open Barqs using file format
            ...
            if (begin_new_session) {
                    ...
                    case Replication::hist_None:
                        ...
                        if (!good_history_type)
                            throw IncompatibleHistories(
                                util::format("Barq file at path '%1' has history type '%2', but is being opened "
                                             "with replication disabled.",
                                             path, Replication::history_type_name(stored_hist_type)),
                                path);
                        break;
                    case Replication::hist_OutOfBarq:
                        LLBT_ASSERT(false); // No longer in use
                        break;
                    case Replication::hist_InBarq:
                        good_history_type = (stored_hist_type == Replication::hist_InBarq ||
                                             stored_hist_type == Replication::hist_None);
                        if (!good_history_type)
                            throw IncompatibleHistories(
                                util::format("Barq file at path '%1' has history type '%2', but is being opened in "
                                             "local history mode.",
                                             path, Replication::history_type_name(stored_hist_type)),
                                path);
                        break;
                ...
                LLBT_ASSERT(stored_hist_schema_version >= 0);
                ...
                // use the same durability setting for the same Barq file.
                ...
                // inconsistency is a logic error, as the user is required to
                // make sure that all possible concurrent session participants
                // use the same history type for the same Barq file.
                ...
                // target Barq file format. From a technical perspective, the
                ...
                // Barq file format version and the same history type, as that
                // would prevent the outcome of the Barq opening process from
    ...
    try {
        ...
        if (current_file_format_version == 0) {
            ...
            // participants must adopt the chosen target Barq file format when
...
void DB::open(Replication& repl, const DBOptions& options)
{
    ...
    m_replication->set_logger(m_logger.get());
    if (m_logger)
        m_logger->detail("Open memory-only barq");
...
// llbt scope cut: history replacement removed with the table layer.
...
// Tracked by https://github.com/barq/barq-core/issues/4111
...
{
    LLBT_ASSERT(!m_fake_read_lock_if_immutable);
...
void DB::release_all_read_locks() noexcept
{
    ...
    LLBT_ASSERT(m_transaction_count == 0);
}
...
public:
    ...
    void blocking_begin_write()
    {
        ...
        LLBT_ASSERT(can_lock_on_caller || m_running || InterprocessMutex::is_thread_confined);
    ...
    {
        std::unique_lock lg(m_mutex);
        LLBT_ASSERT(m_has_write_mutex);
        LLBT_ASSERT(m_owns_write_mutex || !InterprocessMutex::is_thread_confined);
...
void DB::close_internal(std::unique_lock<InterprocessMutex> lock, bool allow_open_read_transactions)
{
    ...
    SharedInfo* info = m_info;
    {
        ...
        if (m_is_sync_agent) {
            LLBT_ASSERT(info->sync_agent_present);
    ...
    while (m_running) {
        ...
        if (m_has_write_mutex) {
            if (auto cb = std::move(m_pending_sync)) {
                ...
                // and pending release.
                LLBT_ASSERT(!m_pending_mx_release);
                lg.unlock();
                cb();
            ...
            if (m_pending_mx_release) {
                LLBT_ASSERT(!InterprocessMutex::is_thread_confined || m_owns_write_mutex);
                m_db->do_end_write();
            ...
            LLBT_ASSERT(!m_pending_sync && !m_pending_mx_release);
            ...
            if (!m_waiting_for_write_mutex && has_pending_write_requests()) {
                ...
                LLBT_ASSERT(!m_pending_writes.empty());
...
bool DB::needs_file_format_upgrade(const std::string& file, Span<const char> encryption_key)
{
    ...
    if (!encryption_key.empty()) {
        LLBT_ASSERT(encryption_key.size() == 64);
                             ...
                             int current_hist_schema_version, int target_hist_schema_version)
{
    ...
    LLBT_ASSERT(current_file_format_version <= target_file_format_version);
    LLBT_ASSERT(current_hist_schema_version <= target_hist_schema_version);
...
// millisleep(200);
        ...
        LLBT_ASSERT(current_hist_schema_version_2 == current_hist_schema_version ||
        ...
        // concurrent DB object.
        LLBT_ASSERT(current_file_format_version_2 == current_file_format_version ||
        ...
        if (need_file_format_upgrade) {
            ...
            // Note: The file format version stored in the Barq file will be
...
void DB::do_end_write() noexcept
{
    ...
    CheckedLockGuard local_lock(m_mutex);
    LLBT_ASSERT(m_write_transaction_open);
    m_alloc.set_read_only(true);
...
Replication::version_type DB::do_commit(Transaction& transaction, bool commit_to_disk)
{
    ...
    // llbt scope cut: async table-clear queue removed with the table layer.
    if (Replication* repl = get_replication()) {
...
{
    auto lockfile_path = get_core_file(llbt_path, CoreFileType::Lock);

    File lockfile;
    lockfile.open(lockfile_path, File::access_ReadWrite, File::create_Auto, 0); // Throws
    File::CloseGuard fcg(lockfile);
    lockfile.set_fifo_path(get_core_file(llbt_path, CoreFileType::Control), "lock.fifo");
...
{
    if (m_fake_read_lock_if_immutable) {
        LLBT_ASSERT(false && "Can't write an immutable DB");
    }
    if (nonblocking) {
...
{
    {
        ...
        LLBT_ASSERT(tr->m_async_stage == Transaction::AsyncState::Idle);
...
void DB::release_sync_agent()
{
    ...
    if (!m_is_sync_agent)
        return;
    LLBT_ASSERT(m_info->sync_agent_present);
