...
{
    llbt::util::encryption_write_barrier(start_addr, size, m_map.get_encrypted_mapping());
}
...
#if LLBT_PLATFORM_APPLE && LLBT_MOBILE
    ...
    , m_free_versions(m_alloc)
{
    ...
    if (ref_type ref = m_free_lengths.get_ref_from_parent()) {
        ...
        LLBT_ASSERT_RELEASE_EX(m_free_positions.size() == m_free_lengths.size(), top.get_ref(),
    ...
    if (ref_type ref = m_free_versions.get_ref_from_parent()) {
        ...
        LLBT_ASSERT_RELEASE_EX(m_free_versions.size() == m_free_lengths.size(), top.get_ref(),
...
void GroupWriter::backdate()
{
    ...
    auto get_earlier = [&](uint64_t version) -> FreeList* {
        ...
        LLBT_ASSERT(it != old_freelists.end());
        LLBT_ASSERT(it != old_freelists.begin());
        --it;
        LLBT_ASSERT((*it)->version < version);
        return it->get();
    };
    ...
    auto find_cover_for = [&](const FreeSpaceEntry& entry, FreeList& free_list) -> std::optional<uint64_t> {
        ...
        LLBT_ASSERT(index < limit); // follows from above
        const auto start_pos = static_cast<ref_type>(free_list.positions.get(index));
        LLBT_ASSERT(start_pos <= entry.ref);
        ...
        LLBT_ASSERT(found_version <= entry.released_at_version);
...
ref_type GroupWriter::write_group()
{
    ...
    // Now, let's update the barq-style freelists, which will later be written to file.
    ...
    LLBT_ASSERT_3(reserve_size, >, max_free_space_needed);
    int_fast64_t value_4 = to_int64(reserve_pos + max_free_space_needed);

#if LLBT_ENABLE_MEMDEBUG
    ...
    LLBT_ASSERT(Array::get_wtype_from_header(Array::get_header_from_data(m_free_versions.m_data)) ==
                 Array::wtype_Bits);
    ...
    if (m_logical_size >= minimal_compaction_size && m_evacuation_limit == 0 && m_backoff == 0) {
        ...
        LLBT_ASSERT(m_logical_size > free_space);
    ...
    LLBT_ASSERT_3(size_t(end_ref), <=, reserve_pos + max_free_space_needed);
    ...
    // expansion
    LLBT_ASSERT_3(value_8, <=, Array::ubound_for_width(m_free_positions.get_width()));
    LLBT_ASSERT_3(value_9, <=, Array::ubound_for_width(m_free_lengths.get_width()));
    ...
    if (m_alloc.is_in_memory()) {
        ...
        LLBT_ASSERT_EX(free_sizes_ref >= reserve_ref && free_sizes_ref + free_sizes_size <= reserve_ref + used,
...
void GroupWriter::read_in_freelist()
{
    ...
    LLBT_ASSERT(m_free_lengths.is_attached());
    size_t limit = m_free_lengths.size();
    LLBT_ASSERT_RELEASE_EX(m_free_positions.size() == limit, limit, m_free_positions.size());
    LLBT_ASSERT_RELEASE_EX(m_free_versions.size() == limit, limit, m_free_versions.size());
...
size_t GroupWriter::recreate_freelist(size_t reserve_pos)
{
    ...
    LLBT_ASSERT(free_in_file.size() == nb_elements);
...

    {
        ...
        for (size_t i = 0; i < limit; ++i) {
            ...
            if (LLBT_UNLIKELY(prev_ref + prev_size > ref)) {
                ...
                for (const auto& elem : new_free_space) {
                    ...
                    for (const auto& locked : m_not_free_in_file) {
                        LLBT_ASSERT_RELEASE_EX(free_ref < locked.ref || free_ref >= (locked.ref + locked.size),
                                                locked.ref, locked.size, locked.released_at_version, free_ref,
                                                m_current_version, m_alloc.get_file_path_for_assertions());
                        LLBT_ASSERT_RELEASE_EX(locked.ref < free_ref || locked.ref >= (free_ref + free_sz),
                                                locked.ref, locked.released_at_version, free_ref, free_sz,
                                                m_current_version, m_alloc.get_file_path_for_assertions());
                    }
                }

                LLBT_ASSERT_RELEASE_EX(prev_ref + prev_size <= ref, prev_ref, prev_size, ref, i, limit,
        ...
        LLBT_ASSERT_RELEASE(reserve_ndx != llbt::npos);
                                                ...
                                                std::multimap<size_t, size_t>& size_map)
{
    ...
    for (auto& elem : list) {
        ...
        if (elem.size) {
            LLBT_ASSERT_RELEASE_EX(!(elem.size & 7), elem.size);
            LLBT_ASSERT_RELEASE_EX(!(elem.ref & 7), elem.ref);
...
size_t GroupWriter::get_free_space(size_t size)
{
    ...
    LLBT_ASSERT_3(chunk_size, >=, size);
    LLBT_ASSERT_RELEASE_EX(!(chunk_pos & 7), chunk_pos);
    LLBT_ASSERT_RELEASE_EX(!(chunk_size & 7), chunk_size);
...
inline GroupWriter::FreeListElement GroupWriter::split_freelist_chunk(FreeListElement it, size_t alloc_pos)
{
    ...
    m_size_map.erase(it);
    LLBT_ASSERT_RELEASE_EX(alloc_pos > start_pos, alloc_pos, start_pos);

    LLBT_ASSERT_RELEASE_EX(!(alloc_pos & 7), alloc_pos);
...
GroupWriter::FreeListElement GroupWriter::extend_free_space(size_t requested_size)
{
    ...
    LLBT_ASSERT_RELEASE_EX(!(new_file_size & 7), new_file_size);
    LLBT_ASSERT_3(logical_file_size, <, new_file_size);
    ...
    LLBT_ASSERT(new_file_size <= get_file_size());
    ...
    LLBT_ASSERT_RELEASE_EX(!(chunk_size & 7), chunk_size);
    LLBT_ASSERT_RELEASE(chunk_size != 0);
...
void GroupWriter::write_array_at(T* translator, ref_type ref, const char* data, size_t size)
{
    ...
    LLBT_ASSERT_3(pos + size, <=, to_size_t(m_group.m_top.get(2) / 2));
    // LLBT_ASSERT_3(pos + size, <=, m_file_map.get_size());
...
void GroupCommitter::commit(ref_type new_top_ref)
{
    ...
    LLBT_ASSERT(!util::int_cast_has_overflow<type_1>(file_format_version));
