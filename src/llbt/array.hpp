...
#ifndef LLBT_ARRAY_HPP
#define LLBT_ARRAY_HPP

#include <llbt/node.hpp>
#include <llbt/query_state.hpp>
#include <llbt/column_fwd.hpp>
...
public:
    ...
    {
        LLBT_ASSERT_3(m_width, ==, get_width_from_header(get_header()));
        return m_width;
    }
    ...
    {
        LLBT_ASSERT_3(m_width, ==, get_width_from_header(get_header()));
        LLBT_ASSERT_3(m_size, ==, get_size_from_header(get_header()));
    ...
    {
        static cond c;
        LLBT_ASSERT(start <= m_size && (end <= m_size || end == size_t(-1)) && start <= end);
        if (end - start == 1) {
            return c(get(start), value) ? start : llbt::not_found;
        }
...
inline int64_t Array::get(size_t ndx) const noexcept
{
    ...
    LLBT_ASSERT_DEBUG_EX(ndx < m_size, ndx, m_size);
    return (this->*m_getter)(ndx);

    // Two ideas that are not efficient but may be worth looking into again:
    /*
        // Assume correct width is found early in LLBT_TEMPEX, which is the case for B tree offsets that
        // are probably either 2^16 long. Turns out to be 25% faster if found immediately, but 50-300% slower
        // if found later
        LLBT_TEMPEX(return get, (ndx));
    */
    /*
...
{
    LLBT_ASSERT_DEBUG(is_attached());
    LLBT_ASSERT_DEBUG_EX(m_has_refs, m_ref, ndx, m_size);
    int64_t v = get(ndx);
    return to_ref(v);
}
...
inline size_t Array::get_byte_size() const noexcept
{
    ...
    LLBT_ASSERT_7(m_alloc.is_read_only(m_ref), ==, true, ||, num_bytes, <=, get_capacity_from_header(header));
