...
{
    LLBT_ASSERT_3(get_wtype_from_header(get_header_from_data(m_data)), ==, wtype_Bits);
...
void Node::alloc(size_t init_size, size_t new_width)
{
    ...
    LLBT_ASSERT_RELEASE(init_size <= max_array_size);

    if (is_read_only())
        do_copy_on_write(needed_bytes);

    LLBT_ASSERT(!m_alloc.is_read_only(m_ref));
