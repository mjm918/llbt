...
#ifndef LLBT_OWNED_DATA_HPP
#define LLBT_OWNED_DATA_HPP
...
public:
    ...
    OwnedData(const char* data_to_copy, size_t data_size)
        : m_size(data_size)
    {
        LLBT_ASSERT_DEBUG(data_to_copy || data_size == 0);
        if (data_to_copy) {
    ...
    OwnedData(std::unique_ptr<char[]> unique_data, size_t data_size) noexcept
        ...
        , m_size(data_size)
    {
        LLBT_ASSERT_DEBUG(m_data || m_size == 0);
    }
