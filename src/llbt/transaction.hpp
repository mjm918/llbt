...
#ifndef LLBT_TRANSACTION_HPP
#define LLBT_TRANSACTION_HPP

#include <llbt/db.hpp>
#include <llbt/impl/changeset_input_stream.hpp>
...
public:
    ...
    // llbt scope cut: table accessors removed.
...
inline bool Transaction::promote_to_write(O* observer, bool nonblocking)
{
    ...
    try {
        ...
        LLBT_ASSERT(repl); // Presence of `repl` follows from the presence of `hist`
...
inline bool Transaction::internal_advance_read(O* observer, VersionID version_id, _impl::History& hist, bool writable)
{
    ...
    LLBT_ASSERT(new_read_lock.m_version >= m_read_lock.m_version);
    ...
    // implemented as a separate unversioned entity outside the Barq (i.e., the
    ...
    // part of the Barq state, then it would not have been necessary to retain
...
{
    LLBT_ASSERT(m_transact_stage != DB::transact_Ready);
    LLBT_ASSERT(end <= m_read_lock.m_version);
    auto hist = get_history(); // Throws
    LLBT_ASSERT(hist);
