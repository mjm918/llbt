...
#ifndef LLBT_ALLOC_HPP
#define LLBT_ALLOC_HPP
...
#if defined LLBT_ARCHITECTURE_X86_32 && defined LLBT_WINDOWS
#define LLBT_WORKAROUND_MSVC_BUG LLBT_NOINLINE
#else
#define LLBT_WORKAROUND_MSVC_BUG
#endif
...
/// The common interface for Barq allocators.
///
/// A Barq allocator must associate a 'ref' to each allocated
...
public:
    LLBT_WORKAROUND_MSVC_BUG inline uint_fast64_t get_content_version() noexcept
    {
...
protected:
    ...
    LLBT_WORKAROUND_MSVC_BUG inline uint_fast64_t get_instance_version() noexcept
    {
...
{
    if (m_is_read_only)
        throw llbt::LogicError(ErrorCodes::WrongTransactionState,
...
inline MemRef Allocator::realloc_(ref_type ref, const char* addr, size_t old_size, size_t new_size)
{
    ...
    if (ref == m_debug_watch)
        LLBT_TERMINATE("Allocator watch: Ref was reallocated");
#endif
    if (m_is_read_only)
        throw llbt::LogicError(ErrorCodes::WrongTransactionState,
                                "Trying to modify database while in read transaction");
...
#ifdef LLBT_DEBUG
    if (ref == m_debug_watch)
        LLBT_TERMINATE("Allocator watch: Ref was freed");
#endif
    LLBT_ASSERT(!m_is_read_only);
...
{
    LLBT_ASSERT_DEBUG(ref != 0);
    // LLBT_ASSERT_DEBUG(m_baseline != 0); // Attached SlabAlloc
...
inline char* Allocator::translate_critical(RefTranslation* ref_translation_ptr, ref_type ref) const noexcept
{
    ...
    if (LLBT_LIKELY(txl.cookie == 0x1234567890)) {
        ...
        if (LLBT_LIKELY(offset < lowest_possible_xover_offset)) {
    ...
    llbt::util::terminate("Invalid ref translation entry", __FILE__, __LINE__, txl.cookie, 0x1234567890, ref, idx);
}

inline char* Allocator::translate(ref_type ref) const noexcept
{
    if (auto ptr = m_ref_translation_ptr.load(std::memory_order_acquire); LLBT_LIKELY(ptr)) {
