...
// These keys must be stored as lowercase. Some names follow existing query type conventions.
...
TypeOfValue::TypeOfValue(const ColKey& col_key)
{
    ...
    LLBT_ASSERT_RELEASE(col_type != col_type_Mixed);
    DataType data_type = DataType(col_type);
    LLBT_ASSERT_RELEASE(data_type.is_valid());
...
std::string TypeOfValue::to_string() const
{
    ...
    while (bit_to_check <= m_attributes) {
        if (m_attributes & bit_to_check) {
            ...
            LLBT_ASSERT_RELEASE_EX(val, bit_to_check);
            values.push_back(*val);
        }
        bit_to_check <<= 1;
    }
    LLBT_ASSERT_RELEASE(values.size() > 0);
    if (values.size() == 1) {
        return values[0];
    }
    else {
