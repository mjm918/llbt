...
{
    return util::format("%1v%2.backup.barq", prefix, version);
}
...
std::string BackupHandler::get_prefix_from_path(const std::string& path)
{
    ...
    // remove a suffix ".barq" but keep the "."
    if (size > 5 && path.substr(size - 5, 5) == ".barq") {
        return path.substr(0, size - 4); // include '.'
    }

    // if no ".barq" suffix, at least ensure a terminating "."
    if (path[size - 1] == '.') {
        return path;
    }
    return path + ".";
...
void BackupHandler::backup_barq_if_needed(int current_file_format_version, int target_file_format_version)
{
    if (current_file_format_version == 0)
        return;
