...
#define LLBT_USE_BUNDLED_SHA2 1
#elif !LLBT_HAVE_OPENSSL
#include <sha1.h>
#define LLBT_USE_BUNDLED_SHA2 1
#endif
 ...
 * LLBT_USE_BUNDLED_SHA2 macro should be undefined.
 * This is particularly relevant when integrating barq-core
 * via vcpkg.
 */
...
#undef LLBT_USE_BUNDLED_SHA2
#endif

#ifdef LLBT_USE_BUNDLED_SHA2
#include <sha224.hpp>
#include <sha256.hpp>
#include <cstring>
#endif
...
struct Algorithm {
    Algorithm(LPCWSTR alg_id)
    {
        if (BCryptOpenAlgorithmProvider(&hAlg, alg_id, NULL, 0) < 0) {
            throw llbt::util::runtime_error("BCryptOpenAlgorithmProvider() failed");
        }
    }
    ~Algorithm()
    {
...
struct Hash {
    ...
    {
        LLBT_ASSERT(alg.obj_length() < 512);
        LLBT_ASSERT(alg.hash_length() == hash_size);
        if (BCryptCreateHash(alg.hAlg, &hHash, hash_object_buffer, 515, NULL, 0, 0) < 0) {
            throw llbt::util::runtime_error("BCryptCreateHash() failed");
        }
    }
    ~Hash()
    {
    ...
    void get_hash(PUCHAR in_buffer, DWORD in_buffer_size, PUCHAR out_buffer)
    {
        if (BCryptHashData(hHash, in_buffer, in_buffer_size, 0) < 0) {
            throw llbt::util::runtime_error("BCryptHashData() failed");
        }
                    ...
                    unsigned char* out_buffer, unsigned int* output_size)
{
    ...
    if (rc == 0) {
        ...
        throw llbt::util::runtime_error("EVP_DigestInit() failed");
    }

    rc = EVP_DigestUpdate(ctx, in_buffer, in_buffer_size);
    if (rc == 0) {
        EVP_MD_CTX_destroy(ctx);
        throw llbt::util::runtime_error("EVP_DigestUpdate() failed");
    }
    ...
    if (rc == 0)
        throw llbt::util::runtime_error("EVP_DigestFinal_ex() failed");
}
#endif
...
void hmac_sha224(Span<const uint8_t> in_buffer, Span<uint8_t, 28> out_buffer, Span<const uint8_t, 32> key)
{
...
#elif defined(LLBT_USE_BUNDLED_SHA2)
    ...
    LLBT_ASSERT_DEBUG(hashLen == out_buffer.size());
#else
