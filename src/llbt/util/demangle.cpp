...
#if LLBT_HAVE_AT_LEAST_GCC(3, 2)
#define LLBT_HAVE_CXXABI_DEMANGLE
#include <cxxabi.h>
#endif
...
std::string llbt::util::demangle(const std::string& mangled_name)
{
#ifdef LLBT_HAVE_CXXABI_DEMANGLE
    int status = 0;
    char* unmangled_name = abi::__cxa_demangle(mangled_name.c_str(), nullptr, nullptr, &status);
    switch (status) {
        case 0:
            LLBT_ASSERT(unmangled_name);
            goto demangled;
        case -1:
            LLBT_ASSERT(!unmangled_name);
