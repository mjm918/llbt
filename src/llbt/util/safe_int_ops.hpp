...
#ifndef LLBT_UTIL_SAFE_INT_OPS_HPP
#define LLBT_UTIL_SAFE_INT_OPS_HPP
...
{
    return llbt::_impl::SafeIntBinops<A, B>::equal(a, b);
}
...
{
    return llbt::_impl::SafeIntBinops<A, B>::less(a, b);
}

template <class A, class B>
constexpr bool int_less_than_or_equal(A a, B b) noexcept
{
    return !llbt::_impl::SafeIntBinops<B, A>::less(b, a); // Not greater than
}

template <class A, class B>
constexpr bool int_greater_than(A a, B b) noexcept
{
    return llbt::_impl::SafeIntBinops<B, A>::less(b, a);
}

template <class A, class B>
constexpr bool int_greater_than_or_equal(A a, B b) noexcept
{
    return !llbt::_impl::SafeIntBinops<A, B>::less(a, b); // Not less than
}
...
constexpr bool int_shift_left_with_overflow_detect(T& lval, int i) noexcept
{
    ...
    LLBT_ASSERT(int_greater_than_or_equal(lval, 0));
    if ((lim::max() >> i) < lval)
        return true;
    lval <<= i;
    return false;
...
constexpr bool int_cast_with_overflow_detect(From from, To& to) noexcept
{
    if (LLBT_LIKELY(!int_cast_has_overflow<To>(from))) {
        to = To(from);
        return false;
    }
    return true;
...
#endif // LLBT_UTIL_SAFE_INT_OPS_HPP
