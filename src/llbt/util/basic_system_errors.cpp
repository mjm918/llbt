...
std::string system_category::message(int value) const
{
    ...
    if (LLBT_LIKELY(strerror_s(buffer, max_msg_size, value) == 0)) {
...

    {
        ...
        if (LLBT_LIKELY(result == 0 || result == ERANGE || result == EINVAL)) {
...
// On Android the GNU variant was only added in API level 23.
    ...
    {
        char* msg = nullptr;
        if (LLBT_LIKELY((msg = strerror_r(value, buffer, max_msg_size)) != nullptr)) {
    ...
    {
        const int result = strerror_r(value, buffer, max_msg_size);
        if (LLBT_LIKELY(result == 0 || result == EINVAL)) {
