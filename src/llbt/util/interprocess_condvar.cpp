...
#include <llbt/util/fifo_helper.hpp>

#include <fcntl.h>
#include <system_error>
#include <sstream>

#ifdef LLBT_CONDVAR_EMULATION
#ifndef _WIN32
#include <unistd.h>
#include <poll.h>
#endif
...
void notify_fd(int fd)
{
    while (true) {
        ...
        LLBT_ASSERT_EX(ret == -1 && errno == EAGAIN, errno);
        millisleep(1);
        continue;
    }
}
                                          ...
                                          std::string tmp_path)
{
    ...
    if (!try_create_fifo(m_resource_path)) {
        ...
        ss << "llbt_" << std::hash<std::string>()(m_resource_path) << ".cv";
    ...
    for (;;) {
        ...
        int r;
        {
            if (tp) {
                ...
                LLBT_ASSERT_DEBUG(!util::int_cast_has_overflow<int64_t>(milliseconds));
...
void InterprocessCondVar::Event::wait(DWORD millis) noexcept
{
    ...
    LLBT_ASSERT_RELEASE(ret == WAIT_OBJECT_0 || ret == WAIT_TIMEOUT);
}
void InterprocessCondVar::Event::set() noexcept
{
    LLBT_ASSERT_RELEASE(SetEvent(handle));
}
void InterprocessCondVar::Event::reset() noexcept
{
    LLBT_ASSERT_RELEASE(ResetEvent(handle));
}
...
void InterprocessCondVar::Mutex::lock() noexcept
{
    ...
    LLBT_ASSERT_RELEASE(ret != WAIT_FAILED);
    LLBT_ASSERT_RELEASE(ret == WAIT_OBJECT_0 || ret == WAIT_ABANDONED_0);
}
...
{
    LLBT_ASSERT_RELEASE(ReleaseMutex(handle));
}

void InterprocessCondVar::update_event_handles()
{
    const int32_t max_process_num = m_shared_part->m_max_process_num;
    LLBT_ASSERT_RELEASE(max_process_num >= 0);
    LLBT_ASSERT_RELEASE(max_process_num < 1'000'000); // Sanity check.
...
InterprocessCondVar::Event InterprocessCondVar::open_event(int32_t n)
{
    LLBT_ASSERT_RELEASE(n >= 0);
    LLBT_ASSERT_RELEASE(n < 1'000'000); // Sanity check.
    const std::string uri = "Local\\llbt_cv_event_" + m_name_with_path + '_' + std::to_string(n);
    const auto wuri = std::wstring(uri.begin(), uri.end());
