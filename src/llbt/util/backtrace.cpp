...
#if !defined(LLBT_HAVE_BACKTRACE) && (LLBT_PLATFORM_APPLE || (defined(__linux__) && defined(__GNUC__)))
// we detect the backtrace facility in CMake, but if building outside it we assume
// it's available on Apple or Linux/glibc
#define LLBT_HAVE_BACKTRACE 1
#endif

#if LLBT_HAVE_BACKTRACE
#if defined(LLBT_BACKTRACE_HEADER)
#include LLBT_BACKTRACE_HEADER
#else
#include <execinfo.h>
#endif
#endif
...
Backtrace Backtrace::capture() noexcept
{
    ...
    if (LLBT_UNLIKELY(frames <= 1)) {
        ...
        if (LLBT_UNLIKELY(memory == nullptr)) {
