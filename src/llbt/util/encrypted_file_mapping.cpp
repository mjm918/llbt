...
#include <llbt/util/aes_cryptor.hpp>
#include <llbt/util/errno.hpp>
#include <llbt/util/sha_crypto.hpp>
...
namespace llbt::util {
// When Barq's file encryption was originally designed, we had the constraint
...
// threading-wise, and is only made safe by Barq's MVCC semantics - if we're
...
// Barq being MVCC with discrete points where we may need to see newer
...
static_assert(metadata_size == 64,
              "changing the size of the metadata breaks compatibility with existing Barq files");
...
{
    To to;
    if (LLBT_UNLIKELY(int_cast_with_overflow_detect(from, to))) {
...
{
    LLBT_ASSERT_DEBUG(!int_cast_has_overflow<To>(from));
    ...
    , m_dst_buffer(new char[encryption_page_size])
{
    ...
    // A random iv is passed to CCCryptorReset. This iv is *not used* by Barq; we set it manually prior to
    ...
    LLBT_ASSERT_RELEASE_EX(ret == 0 && "BCryptOpenAlgorithmProvider()", ret);

    ret = BCryptSetProperty(hAesAlg, BCRYPT_CHAINING_MODE, (PBYTE)BCRYPT_CHAIN_MODE_CBC,
                            sizeof(BCRYPT_CHAIN_MODE_CBC), 0);
    LLBT_ASSERT_RELEASE_EX(ret == 0 && "BCryptSetProperty()", ret);

    ret = BCryptGenerateSymmetricKey(hAesAlg, &m_aes_key_handle, nullptr, 0, (PBYTE)key, 32, 0);
    LLBT_ASSERT_RELEASE_EX(ret == 0 && "BCryptGenerateSymmetricKey()", ret);
#else
...
IVTable& AESCryptor::get_iv_table(FileDesc fd, SizeType data_pos, IVLookupMode mode) noexcept
{
    ...
    LLBT_ASSERT(idx < m_iv_buffer.capacity()); // required space should have been preallocated
...
{
    LLBT_ASSERT(page_ndx < m_iv_buffer.capacity());
...
void AESCryptor::write(FileDesc fd, SizeType pos, const char* src, WriteMarker* marker) noexcept
{
    ...
    do {
        ...
        // they're different.
    } while (LLBT_UNLIKELY(iv.hmac1 == iv.hmac2));
...
void AESCryptor::crypt(EncryptionMode mode, SizeType pos, char* dst, const char* src, const char* stored_iv) noexcept
{
    ...
    LLBT_ASSERT(err == kCCSuccess);
    LLBT_ASSERT(bytesEncrypted == encryption_page_size);
#elif defined(_WIN32)
    ULONG cbData;
    int i;
    ...
    if (mode == mode_Encrypt) {
        ...
        LLBT_ASSERT_RELEASE_EX(i == 0 && "BCryptEncrypt()", i);
        LLBT_ASSERT_RELEASE_EX(cbData == encryption_page_size && "BCryptEncrypt()", cbData);
    }
    else if (mode == mode_Decrypt) {
        i = BCryptDecrypt(m_aes_key_handle, (PUCHAR)src, encryption_page_size, nullptr, (PUCHAR)iv, sizeof(iv),
                          (PUCHAR)dst, encryption_page_size, &cbData, 0);
        LLBT_ASSERT_RELEASE_EX(i == 0 && "BCryptDecrypt()", i);
        LLBT_ASSERT_RELEASE_EX(cbData == encryption_page_size && "BCryptDecrypt()", cbData);
    }
    else {
        LLBT_UNREACHABLE();
    }
...
EncryptedFileMapping::~EncryptedFileMapping()
{
    ...
    LLBT_ASSERT(it != m_file.mappings.end());
...
{
    LLBT_ASSERT_EX(addr >= m_addr, addr, m_addr);
...
{
    LLBT_ASSERT_DEBUG(local_ndx < m_page_state.size());
...
{
    LLBT_ASSERT_EX(local_ndx < m_page_state.size(), local_ndx, m_page_state.size());
    // Precondition: this method must never be called for a page which
    // is already up to date.
    LLBT_ASSERT(is_not(m_page_state[local_ndx], UpToDate));
...
bool EncryptedFileMapping::check_possibly_stale_page(size_t local_ndx) noexcept
{
    ...
    for (auto& m : m_file.mappings) {
        ...
        if (is(state, StaleIV)) {
            LLBT_ASSERT(is_not(state, UpToDate));
            clear(state, StaleIV);
            if (!did_change)
                set(state, UpToDate);
        }
...
{
    LLBT_ASSERT_EX(local_ndx < m_page_state.size(), local_ndx, m_page_state.size());
    LLBT_ASSERT(is_not(m_page_state[local_ndx], Dirty));
    LLBT_ASSERT(is_not(m_page_state[local_ndx], Writable));
...
void EncryptedFileMapping::mark_pages_for_iv_check()
{
    for (auto& state : m_page_state) {
        if (is(state, UpToDate) && is_not(state, Dirty | Writable)) {
            LLBT_ASSERT(is_not(state, StaleIV));
...
{
    LLBT_ASSERT(is(m_page_state[local_ndx], Writable));
    LLBT_ASSERT(is(m_page_state[local_ndx], UpToDate));
    LLBT_ASSERT(is_not(m_page_state[local_ndx], StaleIV));
    LLBT_ASSERT(offset + size <= encryption_page_size);
...
{
#ifdef LLBT_DEBUG
    LLBT_ASSERT(local_ndx < m_page_state.size());
...
void EncryptedFileMapping::write_barrier(const void* addr, size_t size) noexcept
{
    CheckedLockGuard lock(m_file.mutex);
    LLBT_ASSERT(size > 0);
    LLBT_ASSERT(m_access == File::access_ReadWrite);
    ...
    while (size > 0) {
        LLBT_ASSERT(local_ndx < m_page_state.size());
        LLBT_ASSERT(is(m_page_state[local_ndx], PageState::Writable));
...
{
    CheckedLockGuard lock(m_file.mutex);
    LLBT_ASSERT_EX(new_size % encryption_page_size == 0, new_size, encryption_page_size);
...
{
    CheckedLockGuard lock(m_file.mutex);
    LLBT_ASSERT(new_file_offset % encryption_page_size == 0);
    LLBT_ASSERT(new_size % encryption_page_size == 0);
