...
namespace {
bool for_each_helper(const std::string& path, const std::string& dir, llbt::util::File::ForEachHandler& handler)
{
    using File = llbt::util::File;
    llbt::util::DirScanner ds{path}; // Throws
    std::string name;
...
void throwIfCreateDirectoryError(std::error_code error, const std::string& path)
{
    ...
    if (error == errc::permission_denied || error == errc::read_only_file_system) {
        throw llbt::FileAccessError(llbt::ErrorCodes::PermissionDenied, error.message(), path);
    }
    else {
        throw llbt::FileAccessError(llbt::ErrorCodes::FileOperationFailed, error.message(), path);
    }
}
...
std::string make_temp_dir()
{
    ...
    for (;;) {
        if (GetTempFileNameW(temp.c_str(), L"barq", 0, buffer) == 0) {
...
#if LLBT_ANDROID
    std::string buffer = "/data/local/tmp/llbt_XXXXXX";
#else
...
#endif
        LLBT_ASSERT(size > 0 && size % c_min_supported_page_size == 0);
...
void File::close() noexcept
{
    ...
    LLBT_ASSERT_RELEASE(r);
#else // POSIX version
    int r = ::close(m_fd);
    LLBT_ASSERT_RELEASE(r == 0);
#endif
...
size_t File::read_static(FileDesc fd, SizeType pos, char* data, size_t size)
{
    ...
    while (0 < size) {
        ...
        LLBT_ASSERT_RELEASE(size_t(r) <= n);
        size -= size_t(r);
        data += size_t(r);
        pos += r;
    }
...
void File::write_static(FileDesc fd, SizeType pos, const char* data, size_t size)
{
    ...
    while (0 < size) {
        ...
        if (!WriteFile(fd, data, n, &r, &o))
            goto error;
        LLBT_ASSERT_RELEASE(r == n); // Partial writes are not possible.
        size -= size_t(r);
        data += size_t(r);
        pos += r;
    }
...
void File::write(SizeType pos, const char* data, size_t size)
{
    ...
    if (m_encryption) {
        ...
        llbt::util::encryption_write_barrier(write_map, 0, size);
        return;
    }
...
void File::prealloc(SizeType size)
{
    ...
    if (m_encryption) {
        ...
        LLBT_ASSERT(size == encrypted_size_to_data_size(new_size));
    }
...
#if LLBT_HAVE_POSIX_FALLOCATE
    // Mostly Linux only
    if (!prealloc_if_supported(0, new_size)) {
        manually_consume_space();
    }
#elif LLBT_PLATFORM_APPLE // Non-atomic fallback
    ...
    // See <https://github.com/barq/barq-core/issues/3005> for details.
...
#elif LLBT_ANDROID || defined(_WIN32) || defined(__EMSCRIPTEN__)
    manually_consume_space();
#else
#error Please check if/how your OS supports file preallocation
#endif // LLBT_HAVE_POSIX_FALLOCATE
}
...
bool File::prealloc_if_supported(SizeType offset, SizeType size)
{
    ...
    LLBT_ASSERT_RELEASE(is_prealloc_supported());
...
bool File::rw_lock(bool exclusive, bool non_blocking)
{
    ...
    LLBT_ASSERT(!exclusive || non_blocking);

#ifndef LLBT_FILELOCK_EMULATION
    return lock(exclusive, non_blocking);
#else
    LLBT_ASSERT(!m_has_exclusive_lock && !has_shared_lock());
    ...
    LLBT_ASSERT_RELEASE(m_pipe_fd == -1);
    ...
    m_pipe_fd = fd;
    return true;
#endif // LLBT_FILELOCK_EMULATION
}

bool File::lock(bool exclusive, bool non_blocking)
{
    LLBT_ASSERT_RELEASE(is_attached());
    LLBT_ASSERT_RELEASE(!m_have_lock);
    ...
    // the robustness of the Barq API relies in part by the
...
void File::rw_unlock() noexcept
{
    ...
    if (has_shared_lock()) {
        ...
        LLBT_ASSERT(status == 0);
        // close the pipe (== release the shared lock)
        ::close(m_pipe_fd);
        m_pipe_fd = -1;
    }
    else {
        LLBT_ASSERT(m_has_exclusive_lock);
    }
    _unlock(m_fd);
...
{
    LLBT_ASSERT_RELEASE(is_attached());
    LLBT_ASSERT_RELEASE(f.is_attached());
        ...
        if (statbuf.st_size == 0) {
            throw FileAccessError(
                ...
                util::format("Attempt to get unique id on an empty file. This could be due to an external "
                             "process modifying Barq files. '%1'",
                             debug_path),
                debug_path);
        }
...
void File::MapBase::unmap() noexcept
{
    if (!m_addr)
        return;
    LLBT_ASSERT(m_reservation_size);
#if LLBT_ENABLE_ENCRYPTION
...
bool File::MapBase::try_extend_to(size_t size) noexcept
{
    ...
    if (m_encrypted_mapping) {
        ...
        if (got_addr == MAP_FAILED)
            return false;
        LLBT_ASSERT(got_addr == extension_start_addr);
        m_size = size;
...
void File::MapBase::sync()
{
    ...
    llbt::util::msync(m_fd, m_addr, m_size);
}
...
bool DirScanner::next(std::string& name)
{
#if !defined(__linux__) && !LLBT_PLATFORM_APPLE && !LLBT_WINDOWS && !LLBT_UWP && !LLBT_ANDROID &&                \
...
// in 32-bits.
#if LLBT_HAVE_READDIR64
#define LLBT_READDIR(...) readdir64(__VA_ARGS__)
#else
#define LLBT_READDIR(...) readdir(__VA_ARGS__)
#endif

    for (;;) {
        using DirentPtr = decltype(LLBT_READDIR(m_dirp));
        DirentPtr dirent;
        do {
