...
#ifndef LLBT_UTIL_BUFFER_HPP
#define LLBT_UTIL_BUFFER_HPP
...
public:
    ...
    LLBT_NODISCARD std::unique_ptr<T[]> release() noexcept;
...
public:
    ...
    /// usable/logical size.
    LLBT_NODISCARD Buffer<T> release() noexcept;
...
inline void Buffer<T>::resize(size_t new_size, size_t copy_begin, size_t copy_end, size_t copy_to)
{
    ...
    llbt::safe_copy_n(m_data.get() + copy_begin, copy_end - copy_begin, new_data.get() + copy_to);
...
inline void Buffer<T>::reserve(size_t used_size, size_t min_capacity)
{
    ...
    if (LLBT_LIKELY(current_capacity >= min_capacity))
        return;
    size_t new_capacity = current_capacity;

    // Use growth factor 1.5.
    if (LLBT_UNLIKELY(int_multiply_with_overflow_detect(new_capacity, 3)))
        new_capacity = std::numeric_limits<size_t>::max();
    new_capacity /= 2;

    if (LLBT_UNLIKELY(new_capacity < min_capacity))
...
inline void Buffer<T>::reserve_extra(size_t used_size, size_t min_extra_capacity)
{
    ...
    if (LLBT_UNLIKELY(int_add_with_overflow_detect(min_capacity, min_extra_capacity)))
...
inline void AppendBuffer<T>::append(const T* append_data, size_t append_data_size)
{
    ...
    llbt::safe_copy_n(append_data, append_data_size, m_buffer.data() + m_size);
