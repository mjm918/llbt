...
void create_fifo(std::string_view path)
{
    ...
    // file owners. This results in the Android sandbox preventing Barq from opening the
    ...
    // It does make storing Barqs on external storage slightly more unsafe, but users already have
    // full access to modify or copy files from there, so there should be no change from a pratical
    // security standpoint.
    // See more here: https://github.com/barq/barq-java/issues/3972#issuecomment-313675948
    mode_t mode = 0666;
#else
    mode_t mode = 0600;
#endif
