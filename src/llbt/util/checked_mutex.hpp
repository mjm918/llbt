...
#ifndef LLBT_OS_CHECKED_MUTEX_HPP
#define LLBT_OS_CHECKED_MUTEX_HPP
...
#define LLBT_THREAD_ANNOTATION_ATTRIBUTE__(x) __attribute__((x))
#else
#define LLBT_THREAD_ANNOTATION_ATTRIBUTE__(x)
#endif

#define CAPABILITY(x) LLBT_THREAD_ANNOTATION_ATTRIBUTE__(capability(x))

#define RETURN_CAPABILITY(x) LLBT_THREAD_ANNOTATION_ATTRIBUTE__(return_capability(x))

#define SCOPED_CAPABILITY LLBT_THREAD_ANNOTATION_ATTRIBUTE__(scoped_lockable)

#define GUARDED_BY(x) LLBT_THREAD_ANNOTATION_ATTRIBUTE__(guarded_by(x))

#define REQUIRES(...) LLBT_THREAD_ANNOTATION_ATTRIBUTE__(requires_capability(__VA_ARGS__))

#define ACQUIRE(...) LLBT_THREAD_ANNOTATION_ATTRIBUTE__(acquire_capability(__VA_ARGS__))

#define RELEASE(...) LLBT_THREAD_ANNOTATION_ATTRIBUTE__(release_capability(__VA_ARGS__))

#define EXCLUDES(...) LLBT_THREAD_ANNOTATION_ATTRIBUTE__(locks_excluded(__VA_ARGS__))

#define NO_THREAD_SAFETY_ANALYSIS LLBT_THREAD_ANNOTATION_ATTRIBUTE__(no_thread_safety_analysis)
#define ASSERT_CAPABILITY(x) LLBT_THREAD_ANNOTATION_ATTRIBUTE__(assert_capability(x))
...
public:
    ...
    {
        LLBT_ASSERT(lock.owns_lock());
        LLBT_ASSERT(lock.native_handle().mutex() == &m_mutex);
...
#endif // LLBT_OS_CHECKED_MUTEX_HPP
