...
#ifndef LLBT_UTIL_THREAD_HPP
#define LLBT_UTIL_THREAD_HPP
...
protected:
    ...
    LLBT_NORETURN static void init_failed(int);
    LLBT_NORETURN static void attr_init_failed(int);
    LLBT_NORETURN static void destroy_failed(int) noexcept;
    LLBT_NORETURN static void lock_failed(int) noexcept;
...
inline void RobustMutex::lock(Func recover_func)
{
    ...
    if (LLBT_LIKELY(no_thread_has_died))
        return;
    try {
        recover_func(); // Throws
        mark_as_consistent();
...
inline void CondVar::wait(LockGuard& l) noexcept
{
    ...
    if (LLBT_UNLIKELY(r != 0))
        LLBT_TERMINATE("pthread_cond_wait() failed");
#endif
}
