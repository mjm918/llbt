...
#ifndef LLBT_UTIL_SPAN_HPP
#define LLBT_UTIL_SPAN_HPP
...
public:
    ...
    constexpr explicit Span(pointer ptr, size_type count)
        : m_data{ptr}
    {
        LLBT_ASSERT(extent == count);
    }
    constexpr explicit Span(pointer begin, pointer end)
        : m_data{begin}
    {
        LLBT_ASSERT(extent == std::distance(begin, end));
    }
    ...
    {
        LLBT_ASSERT(offset <= size());
        LLBT_ASSERT(count <= size() || count == dynamic_extent);
        if (count == dynamic_extent)
            return {data() + offset, size() - offset};
        LLBT_ASSERT(count <= size() - offset);
...
public:
    ...
    {
        LLBT_ASSERT(offset <= m_size);
        LLBT_ASSERT(count == dynamic_extent || count <= m_size - offset);
    ...
    {
        LLBT_ASSERT(offset <= m_size);
        LLBT_ASSERT(count <= m_size || count == dynamic_extent);
        if (count == dynamic_extent)
            return {m_data + offset, m_size - offset};
        LLBT_ASSERT(count <= m_size - offset);
