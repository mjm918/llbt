...
#ifndef LLBT_IMPL_TRANSACT_LOG_HPP
#define LLBT_IMPL_TRANSACT_LOG_HPP
...
private:
    ...
    LLBT_COLD LLBT_NORETURN void parser_error() const;
...
inline void TransactLogBufferStream::transact_log_reserve(size_t size, char** inout_new_begin, char** out_new_end)
{
    ...
    LLBT_ASSERT(*inout_new_begin >= data);
    LLBT_ASSERT(*inout_new_begin <= (data + m_buffer.size()));
                                                         ...
                                                         char** out_new_end)
{
    ...
    *out_new_begin = llbt::safe_copy_n(data, size, *out_new_begin);
}
...
{
    LLBT_ASSERT(free_begin <= free_end);
...
{
    LLBT_ASSERT_DEBUG(m_transact_log_free_begin <= ptr);
    LLBT_ASSERT_DEBUG(ptr <= m_transact_log_free_end);
...
char* TransactLogEncoder::encode_int(char* ptr, T value)
{
    ...
    if (negative) {
        ...
        LLBT_DIAG_PUSH();
        LLBT_DIAG_IGNORE_UNSIGNED_MINUS();
        value = -(value + 1);
        LLBT_DIAG_POP();
    }
...
void TransactLogParser::parse_one(InstructionHandler& handler)
{
    ...
    switch (instr) {
        ...
        case instr_DictionaryInsert: {
            ...
            LLBT_ASSERT(type == int(type_String));
...
inline StringData TransactLogParser::read_string(std::string& buf)
{
    ...
    constexpr size_t max_string_size = 0xFFFFF8 - Array::header_size - 1; // llbt: was Table::max_string_size
...
#endif // LLBT_IMPL_TRANSACT_LOG_HPP
