...
#ifndef LLBT_NULL_HPP
#define LLBT_NULL_HPP
...
Float/Double: Barq can both store user-given NaNs and null. Any user-given signaling NaN is converted to
0x7fa00000 (if float) or 0x7ff4000000000000 (if double). Any user-given quiet NaN is converted to
0x7fc00000 (if float) or 0x7ff8000000000000 (if double). So Barq does not preserve the optional bits in
user-given NaNs.
...
struct null {
    ...
    /// Returns the quiet NaNs that represent null for floats/doubles in Barq in stored payload.
    template <class T>
    static T get_null_float()
    {
        ...
        LLBT_ASSERT_DEBUG(std::isnan(d));
        LLBT_ASSERT_DEBUG(!is_signaling(d));
        return d;
    }
    ...
    {
        LLBT_ASSERT(std::isnan(static_cast<double>(v)));
        typename std::conditional<std::is_same<T, float>::value, uint32_t, uint64_t>::type i;
