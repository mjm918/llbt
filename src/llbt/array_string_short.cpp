...
{
    LLBT_ASSERT_3(ndx, <, m_size);
    LLBT_ASSERT_3(value.size(), <, max_width); // otherwise we have to use another column type
    ...
    if (m_width <= value.size()) {
        ...
        if (old_width > 0) {
            ...
            while (new_end != base) {
                LLBT_ASSERT_3(new_width, <=, max_width);
...
{
    LLBT_ASSERT_3(ndx, <=, m_size);
    LLBT_ASSERT(value.size() < max_width); // otherwise we have to use another column type
...
void ArrayStringShort::erase(size_t ndx)
{
    ...
    if (ndx < m_size - 1) {
        ...
        llbt::safe_copy_n(old_begin, old_end - old_begin, new_begin);
    }
...
size_t ArrayStringShort::find_first(StringData value, size_t begin, size_t end) const noexcept
{
    ...
    LLBT_ASSERT(begin <= m_size && end <= m_size && begin <= end);
    ...
    if (m_nullable ? value.is_null() : value_size == 0) {
        ...
        for (size_t i = begin; i != end; ++i) {
            ...
            if (LLBT_UNLIKELY(data_i_size == 0))
                return i;
        }
    }
    else {
