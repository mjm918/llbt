...
private:
    ...
    size_t get_elems_per_child() const
    {
        ...
        LLBT_ASSERT_DEBUG(!m_offsets.is_attached());
...
void BPlusTree<int64_t>::split_root()
{
    if (m_root->is_leaf()) {
        ...
        new_root->create(LLBT_MAX_BPNODE_SIZE);

        size_t ndx = 0;
        while (ndx < sz) {
            LeafNode new_leaf(this);
            new_leaf.create();
            size_t to_move = std::min(size_t(LLBT_MAX_BPNODE_SIZE), sz - ndx);
...
void BPlusTreeInner::bptree_access(size_t n, AccessFunc func)
{
    ...
    if (m_offsets.is_attached()) {
        ...
        LLBT_ASSERT_3(child_ndx, <, get_node_size());
    }
    else {
...
size_t BPlusTreeInner::bptree_erase(size_t n, EraseFunc func)
{
    ...
    if (erase_node_size == 0) {
    ...
    else if (erase_node_size < LLBT_MAX_BPNODE_SIZE / 2 && child_ndx < (num_children - 1)) {
        ...
        if (combined_size < LLBT_MAX_BPNODE_SIZE * 3 / 4) {
...
std::unique_ptr<BPlusTreeInner> BPlusTreeInner::split_root()
{
    ...
    size_t elems_per_child = get_elems_per_child();
    new_root->create(LLBT_MAX_BPNODE_SIZE * elems_per_child);
    ...
    LLBT_ASSERT(accumulated_size == tree_size);
...
void BPlusTreeInner::erase_and_destroy_bp_node(size_t ndx)
{
    ...
    LLBT_ASSERT_DEBUG(m_offsets.size() == get_node_size() - 1);
}
...
void BPlusTreeInner::verify() const
{
    ...
    LLBT_ASSERT_3(Array::get_type(), ==, Array::type_InnerBptreeNode);

    LLBT_ASSERT_3(Array::size(), >=, 2);
    size_t num_children = get_node_size();

    // Verify invar:bptree-nonempty-inner
    LLBT_ASSERT_3(num_children, >=, 1);

    size_t elems_per_child = 0;
    if (m_offsets.is_attached()) {
        LLBT_ASSERT_DEBUG(m_offsets.size() == num_children - 1);
    }
    else {
    ...
    for (size_t i = 0; i < num_children; i++) {
        ...
        if (m_offsets.is_attached()) {
            if (i < num_children - 1) {
                LLBT_ASSERT_DEBUG(num_elems == m_offsets.get(i));
            }
        }
        else {
            if (i < num_children - 1) {
                LLBT_ASSERT_DEBUG(elems_in_child == elems_per_child);
            }
            else {
                LLBT_ASSERT_DEBUG(elems_in_child <= elems_per_child);
            }
    ...
    LLBT_ASSERT_DEBUG(get_tree_size() == num_elems);
    m_tree->invalidate_leaf_cache();
#endif
}
...
{
    LLBT_ASSERT(m_size == 0 && m_root && m_root->get_tree_size() == 0);
    if (level.empty())
        return;
    ...
    size_t elems_per_child = LLBT_MAX_BPNODE_SIZE;
    std::vector<ref_type> next;
    while (level.size() > 1) {
        next.clear();
        next.reserve((level.size() + LLBT_MAX_BPNODE_SIZE - 1) / LLBT_MAX_BPNODE_SIZE);
        for (size_t i = 0; i < level.size(); i += LLBT_MAX_BPNODE_SIZE) {
            size_t n = std::min(level.size() - i, size_t(LLBT_MAX_BPNODE_SIZE));
        ...
        level.swap(next);
        elems_per_child *= LLBT_MAX_BPNODE_SIZE;
    }
