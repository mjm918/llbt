...
#ifndef LLBT_MIXED_HPP
#define LLBT_MIXED_HPP
...
#include <llbt/data_type.hpp>
#if LLBT_ENABLE_GEOSPATIAL
#include <llbt/geospatial.hpp>
#endif
...
enum JSONOutputMode {
    output_mode_json,       // default / existing implementation for outputting barq to json
...
/// This class represents a polymorphic Barq value.
///
...
/// data) is stored in a Barq database, it is always followed by a
...
{
    LLBT_ASSERT(get_type() == type_Bool);
    return bool_val;
}
...
{
    LLBT_ASSERT(get_type() == type_Float);
    return float_val;
}
...
{
    LLBT_ASSERT(get_type() == type_Double);
    return double_val;
}
...
inline StringData Mixed::get<StringData>() const noexcept
{
    if (is_null())
        return StringData();
    LLBT_ASSERT(get_type() == type_String);
    return string_val;
}
...
inline BinaryData Mixed::get<BinaryData>() const noexcept
{
    if (is_null())
        return BinaryData();
    LLBT_ASSERT(get_type() == type_Binary);
    return binary_val;
}
...
{
    LLBT_ASSERT(get_type() == type_Timestamp);
    return date_val;
}
...
{
    LLBT_ASSERT(get_type() == type_Decimal);
    return decimal_val;
}
...
{
    LLBT_ASSERT(get_type() == type_ObjectId);
    return id_val;
}
...
{
    LLBT_ASSERT(get_type() == type_UUID);
    return uuid_val;
}
...
inline ObjKey Mixed::get<ObjKey>() const noexcept
{
    ...
    LLBT_ASSERT(get_type() == type_Link);
...
{
    LLBT_ASSERT(get_type() == type_TypedLink);
    return link_val;
}
...
{
    auto type = get_type();
    LLBT_ASSERT_EX(type == type_Geospatial, type);
...
namespace std {
template <>
struct hash<::llbt::Mixed> {
    inline size_t operator()(const ::llbt::Mixed& m) const noexcept
    {
        return m.hash();
    }
};
