...
/// remain zero.
class DefaultAllocator : public llbt::Allocator {
public:
    DefaultAllocator()
    {
        m_baseline = 0;
    ...
    MemRef do_alloc(const size_t size) override
    {
        ...
        if (LLBT_UNLIKELY(LLBT_COVER_NEVER(!addr))) {
            // LCOV_EXCL_START
            LLBT_ASSERT_DEBUG(errno == ENOMEM);
            throw std::bad_alloc();
            // LCOV_EXCL_STOP
        }
    ...
    MemRef do_realloc(ref_type, char* addr, size_t old_size, size_t new_size) override
    {
        ...
        if (LLBT_UNLIKELY(LLBT_COVER_NEVER(!new_addr))) {
            // LCOV_EXCL_START
            LLBT_ASSERT_DEBUG(errno == ENOMEM);
            throw std::bad_alloc();
            // LCOV_EXCL_STOP
...
char* Allocator::translate_less_critical(RefTranslation* ref_translation_ptr, ref_type ref) const noexcept
{
    ...
    if (LLBT_LIKELY(!crosses_mapping)) {
