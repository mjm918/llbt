...
template <typename T>
constexpr bool is_status_or_status_with = std::is_same_v<T, ::llbt::Status> || is_status_with<T>;
 ...
 */
template <typename T>
class LLBT_NODISCARD StatusWith {
...
public:
    ...
    {
        LLBT_ASSERT_DEBUG(is_ok());
        LLBT_ASSERT_RELEASE(m_value);
        return *m_value;
    }
