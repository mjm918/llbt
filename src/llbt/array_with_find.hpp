...
#ifndef LLBT_ARRAY_WITH_FIND_HPP
#define LLBT_ARRAY_WITH_FIND_HPP
...
#include <llbt/llbt_nmmintrin.h> // SSE42
#endif
...
// SSE find for the four functions Equal/NotEqual/Less/Greater
    ...
    template <class cond, size_t width>
    LLBT_FORCEINLINE bool find_sse_intern(__m128i* action_data, __m128i* data, size_t items, QueryStateBase* state,
...
uint64_t ArrayWithFind::cascade(uint64_t a) const
{
    ...
    if (width == 1) {
        ...
        LLBT_ASSERT_DEBUG(false);
        return uint64_t(-1);
    }
}

template <size_t bitwidth>
LLBT_NOINLINE bool ArrayWithFind::find_all_will_match(size_t start2, size_t end, size_t baseindex,
                                                       QueryStateBase* state) const
{
    LLBT_ASSERT_DEBUG(state->match_count() < state->limit());
...
bool ArrayWithFind::find_optimized(int64_t value, size_t start, size_t end, size_t baseindex,
                                   QueryStateBase* state) const
{
    LLBT_ASSERT_DEBUG(start <= m_array.m_size && (end <= m_array.m_size || end == size_t(-1)) && start <= end);
    ...
    LLBT_ASSERT_3(m_array.m_width, !=, 0);

#if defined(LLBT_COMPILER_SSE)
...
size_t ArrayWithFind::find_zero(uint64_t v) const
{
    ...
    while (eq == (((v >> (width * start)) & mask) != 0)) {
        ...
        LLBT_ASSERT_3(start, <=, 8 * sizeof(v));
        start++;
    }
...
{
    LLBT_TEMPEX2(return find_optimized, cond, m_array.m_width, (value, start, end, baseindex, state));
}
...
template <class cond, size_t width>
LLBT_FORCEINLINE bool ArrayWithFind::find_sse_intern(__m128i* action_data, __m128i* data, size_t items,
                                  ...
                                  QueryStateBase* state) const
{
    ...
    bool r;
    LLBT_TEMPEX2(r = compare_leafs, cond, m_array.m_width, (foreign, start, end, baseindex, state))
    return r;
}
                                  ...
                                  QueryStateBase* state) const
{
    ...
    bool r;
    LLBT_TEMPEX3(r = compare_leafs_4, cond, width, fw, (foreign, start, end, baseindex, state))
    return r;
}
...
bool ArrayWithFind::compare_relation(int64_t value, size_t start, size_t end, size_t baseindex,
                                     QueryStateBase* state) const
{
    LLBT_ASSERT(start <= m_array.m_size && (end <= m_array.m_size || end == size_t(-1)) && start <= end);
...
#endif /* LLBT_ARRAY_WITH_FIND_HPP */
