...
LLBT_FORCEINLINE bool matchlike(const StringData& text, const StringData& pattern,
                                 const StringData* alternate_pattern = nullptr) noexcept
{
    ...
    LLBT_ASSERT_DEBUG(has_alternate_pattern == bool(alternate_pattern));
    LLBT_ASSERT_DEBUG(!alternate_pattern || pattern.size() == alternate_pattern->size());
...
bool StringData::matchlike(const llbt::StringData& text, const llbt::StringData& pattern) noexcept
{
...
size_t llbt::murmur2_or_cityhash(const unsigned char* data, size_t len) noexcept
{
    return size_t(Murmur2OrCityHash<>{}(data, len));
}

uint_least32_t llbt::murmur2_32(const unsigned char* data, size_t len) noexcept
{
    // This implementation is copied from libc++.
    // See: https://github.com/llvm-mirror/libcxx/blob/master/include/utility

    LLBT_ASSERT_DEBUG(len <= std::numeric_limits<uint_least32_t>::max());
...
uint_least64_t llbt::cityhash_64(const unsigned char* data, size_t len) noexcept
{
