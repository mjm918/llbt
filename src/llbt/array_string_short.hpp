...
#ifndef LLBT_ARRAY_STRING_SHORT_HPP
#define LLBT_ARRAY_STRING_SHORT_HPP
...
xxx0 xx01 x002 0003 0004 (strings "xxx",. "xx", "x", "", llbt::null())

So each string is 0 terminated, and the last byte in a block tells how many 0s are present, except
for a llbt::null() which has the byte set to m_width (4). The byte is used to compute the length of a string
in various functions.

New: If m_witdh = 0, then all elements are llbt::null(). So to add an empty string we must expand m_width
...
inline StringData ArrayStringShort::get(size_t ndx) const noexcept
{
    ...
    if (m_width == 0)
        return m_nullable ? llbt::null() : StringData("");
