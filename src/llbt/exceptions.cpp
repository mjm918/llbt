...
#include <llbt/util/demangle.hpp>
...
LogicError::LogicError(ErrorCodes::Error code, std::string_view msg)
    : Exception(code, msg)
{
    LLBT_ASSERT(ErrorCodes::error_categories(code).test(ErrorCategory::logic_error));
}
...
RuntimeError::RuntimeError(ErrorCodes::Error code, std::string_view msg)
    : Exception(code, msg)
{
    LLBT_ASSERT(ErrorCodes::error_categories(code).test(ErrorCategory::runtime_error));
}
RuntimeError::RuntimeError(Status&& status)
    : Exception(std::move(status))
{
    LLBT_ASSERT(ErrorCodes::error_categories(to_status().code()).test(ErrorCategory::runtime_error));
}
...
InvalidArgument::InvalidArgument(ErrorCodes::Error code, std::string_view msg)
    : LogicError(code, msg)
{
    LLBT_ASSERT(ErrorCodes::error_categories(code).test(ErrorCategory::invalid_argument));
}
...
FileAccessError::FileAccessError(ErrorCodes::Error code, std::string_view msg, std::string_view path, int err)
    ...
    , m_errno(err)
{
    LLBT_ASSERT(ErrorCodes::error_categories(code).test(ErrorCategory::file_access));
}
