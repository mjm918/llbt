...
#include <llbt.hpp>
#include <llbt/array_decimal128.hpp>
...
TEST(Decimal_Array)
{
    ...
    arr.create();
    arr.add(Decimal128(llbt::null()));
    CHECK_EQUAL(arr.get_width(), 0);
    CHECK_EQUAL(arr.get(0), Decimal128(llbt::null()));
    CHECK(arr.is_null(0));
    arr.add(Decimal128());
    ...
    CHECK_EQUAL(arr.get_width(), 4);
    CHECK_EQUAL(arr.get(0), Decimal128());
    CHECK_EQUAL(arr.get(1), Decimal128(llbt::null()));
    CHECK_NOT(arr.is_null(0));
    CHECK(arr.is_null(1));
    ...
    CHECK_EQUAL(arr.find_first(Decimal128(llbt::null())), 3);
    ...
    CHECK_EQUAL(arr1.get(1), Decimal128(llbt::null()));
    ...
    CHECK_EQUAL(arr.get(2), Decimal128(llbt::null()));
    CHECK_EQUAL(arr1.get(0), Decimal128(str1));
...
// llbt scope cut: needs the removed table layer.
#if LLBT_TEST_HAVE_TABLES
TEST(Decimal_Table)
{
    const char str0[] = "12345.67";
    const char str1[] = "1000.00";
...
TEST(Decimal_Query)
{
    ...
    }
    {
        ...
        Query q2 = table->column<Decimal>(col) == llbt::null();
        CHECK_EQUAL(q2.count(), 1);
        q2 = table->where().equal(col, llbt::null());
        CHECK_EQUAL(q2.count(), 1);
