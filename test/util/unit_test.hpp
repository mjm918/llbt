...
#ifndef LLBT_TEST_UTIL_UNIT_TEST_HPP
#define LLBT_TEST_UTIL_UNIT_TEST_HPP
...
#include <llbt/util/bind_ptr.hpp>
...
/// TestList::run().
#define TEST_IF(name, enabled) TEST_EX(name, llbt::test_util::unit_test::get_default_test_list(), enabled, true)
...
#define NONCONCURRENT_TEST_IF(name, enabled)                                                                         \
    TEST_EX(name, llbt::test_util::unit_test::get_default_test_list(), enabled, false)

#define TEST_EX(name, list, enabled, allow_concur)                                                                   \
    struct Barq_UnitTest__##name : llbt::test_util::unit_test::TestBase {                                          \
        static bool test_enabled()                                                                                   \
        {                                                                                                            \
            return bool(enabled);                                                                                    \
        }                                                                                                            \
        Barq_UnitTest__##name(llbt::test_util::unit_test::TestContext& c)                                          \
            : TestBase(c)                                                                                            \
        {                                                                                                            \
        }                                                                                                            \
        void test_run();                                                                                             \
    };                                                                                                               \
    llbt::test_util::unit_test::RegisterTest<Barq_UnitTest__##name> llbt_unit_test_reg__##name(                   \
        (list), (allow_concur), "DefaultSuite", #name, __FILE__, __LINE__);                                          \
    void Barq_UnitTest__##name::test_run()
...
template <typename T>
constexpr static bool llbt_serializable_types =
    is_any_v<T, StringData, BinaryData, Timestamp, ObjectId, std::optional<ObjectId>, ObjKey, ObjLink, UUID,
             std::optional<UUID>, bool, float, std::optional<float>, double, std::optional<double>, llbt::null>;

template <class T>
void to_string(const T& value, std::string& str)
{
    if constexpr (llbt_serializable_types<T>) {
...
#endif // LLBT_TEST_UTIL_UNIT_TEST_HPP
