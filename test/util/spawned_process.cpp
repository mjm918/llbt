...
{
    const char* str = getenv("LLBT_CHILD_IDENT");
    return str && str == m_identifier;
}

bool SpawnedProcess::is_parent()
{
    return !(getenv("LLBT_CHILD_IDENT") || getenv("LLBT_SPAWNED"));
}
...
int SpawnedProcess::wait_for_child_to_finish()
{
    ...
    LLBT_ASSERT_RELEASE_EX(ret != -1, errno, m_pid, m_test_name, m_identifier);

    bool signaled_to_stop = WIFSIGNALED(status);
    LLBT_ASSERT_RELEASE_EX(!signaled_to_stop, WTERMSIG(status), WCOREDUMP(status), m_pid, m_test_name, m_identifier);

    bool stopped = WIFSTOPPED(status);
    LLBT_ASSERT_RELEASE_EX(!stopped, WSTOPSIG(status), m_pid, m_test_name, m_identifier);

    bool exited_normally = WIFEXITED(status);
    LLBT_ASSERT_RELEASE_EX(exited_normally, m_pid, m_test_name, m_identifier);

    auto exit_status = WEXITSTATUS(status);
    LLBT_ASSERT_RELEASE_EX(exit_status == 0, exit_status, m_pid, m_test_name, m_identifier);
    return status;
#else
    if (!is_parent()) {
        return 0;
    }
    LLBT_ASSERT_EX(m_process.hProcess != 0, m_test_name, m_identifier);
    ...
    if (status == WAIT_TIMEOUT) {
        LLBT_ASSERT_EX(false, "process wait failed", m_test_name, m_identifier);
    }
    else if (status == WAIT_FAILED) {
        LLBT_ASSERT_EX(false, util::format("process wait failed (%1)", GetLastError()), m_test_name, m_identifier);
    }
    LLBT_ASSERT_EX(status == WAIT_OBJECT_0, status, m_test_name, m_identifier);
    return 0;
#endif
}
...
std::unique_ptr<SpawnedProcess> spawn_process(const std::string& test_name, const std::string& process_ident)
{
    ...
    const char* child_ident = getenv("LLBT_CHILD_IDENT");
    if (child_ident) {
    ...
    std::vector<std::string> env_vars = {"LLBT_SPAWNED=1", util::format("UNITTEST_FILTER=%1", test_name),
                                         util::format("LLBT_CHILD_IDENT=%1", process_ident)};
    ...
    LLBT_ASSERT_EX(success, util::format("GetModuleFileName failed (%1)", GetLastError()), test_name, process_ident);
    std::string env;
    ...
    if (!CreateProcess(program_name, // Application name
        ...
        LLBT_ASSERT_EX(false, util::format("CreateProcess failed (%1).\n", GetLastError()), test_name,
                        process_ident);
    }
    process->set_pid(pi);
#else
    ...
    // need to use the same test path as parent so that tests use the same barq paths
