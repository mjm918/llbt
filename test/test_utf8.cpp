...
{
    // Test that invalid utf8 won't make decisions on data beyond Barq payload. Do
    // that by placing an utf8 header that indicate 5 octets will follow, and put
    // spurious1 and spurious2 after them to see if Barq will access these too and
...
{
    // See if we can crash Barq with random data
...
TEST(UTF8_Compare_Core_utf8_zero)
{
    ...
    // Barq must support 0 characters in utf8 strings
