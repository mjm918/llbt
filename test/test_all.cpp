...
#include <llbt/util/platform_info.hpp>
#include <llbt.hpp>
...
#if !defined _WIN32 && !LLBT_PLATFORM_APPLE
...
using namespace llbt::test_util::unit_test;
...
#ifdef LLBT_COMPILER_AVX
    const char* compiler_avx = "Yes";
#else
    const char* compiler_avx = "No";
#endif

    const char* cpu_sse = llbt::sseavx<42>() ? "4.2" : (llbt::sseavx<30>() ? "3.0" : "None");

    const char* cpu_avx = llbt::sseavx<1>() ? "Yes" : "No";

    std::cout << std::endl
              << "Barq version: " << Version::get_version() << " with Debug " << with_debug << "\n"
              << "Platform: " << util::get_platform_info() << "\n"
              << "Encryption: " << encryption << "\n"
              << "\n"
              << "LLBT_MAX_BPNODE_SIZE = " << LLBT_MAX_BPNODE_SIZE << "\n"
              << "LLBT_MEMDEBUG = " << memdebug << "\n"
              << "\n"
...
bool run_tests(const std::shared_ptr<llbt::util::Logger>& logger = nullptr)
{
    {
    ...
    const bool running_spawned_process = getenv("LLBT_SPAWNED");
    ...
    if (const char* str = getenv("UNITTEST_XML"); str && strlen(str) != 0) {
        ...
        if (!test_suite_name || !strlen(test_suite_name))
            test_suite_name = "barq-core-tests";
...
int test_all(const std::shared_ptr<llbt::util::Logger>& logger)
{
    // General note: Some Github clients on Windows will interfere with the .barq files created by unit tests (the
    ...
    // (not the whole platform) may produce corrupt barq files.
    ...
    if (num_open_files >= 0) {
        ...
        LLBT_ASSERT(num_open_files_2 >= 0);
