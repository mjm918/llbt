...
TEST(StringData_Hash_Murmur2)
{
    ...
    const uint_least32_t hash = llbt::murmur2_32(reinterpret_cast<const unsigned char*>(input.data()), input.size());
...
TEST(StringData_Hash_CityHash64)
{
    ...
    for (size_t i = 0; i < num_checks; ++i) {
        ...
        const uint_least64_t hash = llbt::cityhash_64(data, input_expected[i].first.size());
